 ...
 * @mapped_channel_id:	KSLC PKTDMA thread to channel mapping for mapped
 *			channels. The thread must be serviced by the specified
 *			channel if mapped_channel_id is >= 0 in case of PKTDMA
 * @flow_start:		KSLC PKDMA flow range start of mapped channel. Unmapped
 *			channels use flow_id == chan_id
 * @flow_num:		KSLC PKDMA flow count of mapped channel. Unmapped
 *			channels use flow_id == chan_id
 * @default_flow_id:	KSLC PKDMA default (r)flow index of mapped channel.
...
struct psil_endpoint_config {
	...
	/* KSLC PKDMA mapped channel */
	int mapped_channel_id;
	/* KSLC PKTDMA tflow and rflow ranges for mapped channel */
	u16 flow_start;
	u16 flow_num;
	u16 default_flow_id;
};
