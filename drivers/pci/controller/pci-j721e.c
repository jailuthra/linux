...
/*
 * pci-j721e - PCIe controller driver for TI J721E SoCs
 *
...
static void j721e_pcie_quirk(struct pci_dev *pci_dev)
{
	...
	if (pci_match_id(rc_pci_devids, bridge)) {
		...
		val = (bus->number << 8 | pci_dev->devfn) & J721E_REQID_MASK;
...
static int __init j721e_pcie_probe(struct platform_device *pdev)
{
...
static int __exit j721e_pcie_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
...
static struct platform_driver j721e_pcie_driver = {
	...
	.remove = __exit_p(j721e_pcie_remove),
	.driver = {
		.name	= "j721e-pcie",
