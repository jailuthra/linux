...
static int dmatest_func(void *data)
{
	...
	chan = thread->chan;
	dev = chan->device;
	mapping_dev = dmaengine_get_device_for_dma_api(chan);
	...
	while (!(kthread_should_stop() ||
		...
		um = dmaengine_get_unmap_data(mapping_dev, src->cnt + dst->cnt,
					      GFP_KERNEL);
		if (!um) {
			failed_tests++;
		...
		for (i = 0; i < src->cnt; i++) {
			...
			um->addr[i] = dma_map_page(mapping_dev, pg, pg_off,
						   um->len, DMA_TO_DEVICE);
			srcs[i] = um->addr[i] + src->off;
			ret = dma_mapping_error(mapping_dev, um->addr[i]);
			if (ret) {
		...
		for (i = 0; i < dst->cnt; i++) {
			...
			dsts[i] = dma_map_page(mapping_dev, pg, pg_off, um->len,
					       DMA_BIDIRECTIONAL);
			ret = dma_mapping_error(mapping_dev, dsts[i]);
			if (ret) {
