...
#define IMX219_COMPAT_ACTIVE_AREA_TOP	8U
/* IMX219 native and active pixel array size. */
#define IMX219_INACTIVE_AREA_HEIGHT	42U
#define IMX219_ACTIVE_AREA_LEFT		8U
#define IMX219_ACTIVE_AREA_TOP		(IMX219_INACTIVE_AREA_HEIGHT + 8U)
#define IMX219_ACTIVE_AREA_WIDTH	3280U
#define IMX219_ACTIVE_AREA_HEIGHT	2464U
#define IMX219_PIXEL_ARRAY_LEFT		0U
#define IMX219_PIXEL_ARRAY_TOP		IMX219_INACTIVE_AREA_HEIGHT
				...
				struct v4l2_rect *crop, unsigned int index)
{
	...
	crop->width = clamp(crop->width, IMX219_OUTPUT_X_SIZE_MIN *
			    ...
			    IMX219_ACTIVE_AREA_WIDTH) & ~1;
	crop->height = clamp(crop->height, IMX219_OUTPUT_Y_SIZE_MIN *
			     V4L2_BINNING_FACTORS_VNUM(binning),
			     IMX219_ACTIVE_AREA_HEIGHT) & ~1;
	crop->left = clamp((unsigned int)crop->left, IMX219_ACTIVE_AREA_LEFT,
			   IMX219_ACTIVE_AREA_LEFT + IMX219_ACTIVE_AREA_WIDTH -
			   crop->width);
	crop->top = clamp((unsigned int)crop->top, IMX219_ACTIVE_AREA_TOP,
			  IMX219_ACTIVE_AREA_TOP + IMX219_ACTIVE_AREA_HEIGHT -
			  crop->height);
	...
	int vblank_min = IMX219_VBLANK_MIN +
		((int)source_format->height *
		 (1 - (int)V4L2_BINNING_FACTORS_VNUM(binning)) /
		 (int)V4L2_BINNING_FACTORS_VNUM(binning));
				 ...
				 u64 streams_mask)
{
	...
	__v4l2_ctrl_grab(imx219->binning, true);
				  ...
				  u64 streams_mask)
{
	...
	__v4l2_ctrl_grab(imx219->binning, false);
				  ...
				  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	case IMX219_PAD_IMAGE:
		...
		fse->max_width = IMX219_ACTIVE_AREA_WIDTH;
		fse->min_height = IMX219_ACTIVE_AREA_HEIGHT;
		fse->max_height = IMX219_ACTIVE_AREA_HEIGHT;
		return 0;
				...
				struct v4l2_subdev_selection *sel)
{
	if (!(ci &&
		...
		case V4L2_SEL_TGT_CROP_BOUNDS:
			sel->r.top = IMX219_COMPAT_ACTIVE_AREA_TOP;
			     ...
			     struct v4l2_subdev_state *state)
{
	...
	pixel_array_format->width = IMX219_ACTIVE_AREA_WIDTH;
	pixel_array_format->height = IMX219_ACTIVE_AREA_HEIGHT;
	...
	analogue_crop->top = IMX219_ACTIVE_AREA_TOP;
	analogue_crop->left = IMX219_ACTIVE_AREA_LEFT;
	analogue_crop->width = IMX219_ACTIVE_AREA_WIDTH;
	analogue_crop->height = IMX219_ACTIVE_AREA_HEIGHT;
