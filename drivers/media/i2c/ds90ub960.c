...
static const struct ub960_format_info ub960_formats[] = {
	...
	/* Metadata hacks */
	{ .code = MEDIA_BUS_FMT_META_8, .bpp = 8, .datatype = MIPI_CSI2_DT_EMBEDDED_8B, .meta = true, },
	{ .code = MEDIA_BUS_FMT_META_12, .bpp = 12, .datatype = MIPI_CSI2_DT_EMBEDDED_8B, .meta = true },
};
...
static int ub960_rxport_serializer_write(struct ub960_rxport *rxport, u8 reg, u8 val)
{
	...
	if (ret)
		dev_dbg(dev,
			"%s: cannot write back-channel register 0x%02x (%d)!\n",
			__func__, reg, ret);

	return ret;
}

static void ub960_rxport_bc_ser_config(struct ub960_rxport *rxport)
{
	struct ub960_data *priv = rxport->priv;
	struct device *dev = &priv->client->dev;
	...
	/* Freeze serializer's I2C address to prevent updates from FC */
	ub960_rxport_write(priv, nport, UB960_RR_SER_ID,
			   rxport->ser.addr << 1 |
				   UB960_RR_SER_ID_FREEZE_DEVICE_ID, &ret);

	/*
	 * Enable I2C auto ack for cases where serializer strap values
	 * are wrong and FC is not functional.
	 */
	...
	if (ret)
		dev_err(dev,
			"rx%u: Failed to enable I2C passthrough on BC: %d\n",
			nport, ret);

	/* Disable BC alternate mode auto detect */
	ub960_rxport_serializer_write(rxport, 0x4b, 0x02);
	/* Decrease link detect timer */
	ub960_rxport_serializer_write(rxport, 0x49, 0x06);
	...
	/* Disable auto-ack for the remote serializer */
	ub960_rxport_update_bits(priv, nport, UB960_RR_SER_ALIAS_ID,
				 UB960_RR_SER_ALIAS_ID_AUTO_ACK, 0x0, &ret);

	if (ret)
		dev_err(dev,
			"rx%u: Failed to disable I2C auto-ack and passthrough: %d\n",
			nport, ret);
}
...
static int ub960_init_tx_ports(struct ub960_data *priv)
{
	...
	for (unsigned int nport = 0; nport < priv->hw_data->num_txports; nport++) {
	...
	for (unsigned int retry = 0; retry < 3; ++retry) {
		...
		if (!rx_lock) {
			...
			dev_dbg(dev, "rx%u: no lock, retry = %u\n", nport,
				retry);
		} else if (rx_lock && rx_aeq >= rx_aeq_limit) {
			if (rx_aeq != prev_aeq) {
				ret = ub960_rxport_lockup_wa_ub9702(priv);
				if (ret)
					return ret;
...
static int ub960_init_rx_ports_ub9702(struct ub960_data *priv)
{
	...
	for_each_active_rxport(priv) {
		if (fwnode_device_is_compatible(it.rxport->ser.fwnode,
						"ti,ds90ub971-q1")) {
			ub960_rxport_bc_ser_config(it.rxport);
		}
	}
...
static int ub960_validate_stream_vcs(struct ub960_data *priv)
{
	for_each_active_rxport(priv) {
		...
		ret = v4l2_subdev_call(it.rxport->source.sd, pad, get_frame_desc,
				       it.rxport->source.pad, &desc);
		if (ret)
			return ret;
