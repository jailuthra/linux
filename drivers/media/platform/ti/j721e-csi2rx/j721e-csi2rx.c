...
#define TI_CSI2RX_MAX_PIXELS_PER_CLOCK	4
...
static void ti_csi2rx_request_max_ppc(struct ti_csi2rx_dev *csi)
{
	...
	ppc = TI_CSI2RX_MAX_PIXELS_PER_CLOCK;
	ret = cdns_csi2rx_negotiate_ppc(csi->source, pad->index, &ppc);
	if (ret) {
		dev_warn(csi->dev,
			 "Unable to negotiate pixel-per-clock, defaulting to 1");
		csi->pix_per_clk = 1;
		return;
	}
...
static void ti_csi2rx_setup_shim(struct ti_csi2rx_ctx *ctx)
{
	...
	if (!csi->enable_count) {
		/* Negotiate pixel count before starting first stream on source */
