...
static void HEVC_SWSR_U1(char *what, u8 *where, void *swsr_ctx)
{
	*where = swsr_read_bits(swsr_ctx, 1);
	pr_info("%s, u(1) : %u", what, *where);
}

static void HEVC_SWSR_UN(char *what, u32 *where, u8 numbits, void *swsr_ctx)
{
	*where = swsr_read_bits(swsr_ctx, numbits);
	pr_info("%s, u(%u) : %u", what, numbits, *where);
}

static void HEVC_SWSR_UE(char *what, u32 *where, void *swsr_ctx)
{
	*where = swsr_read_unsigned_expgoulomb(swsr_ctx);
	pr_info("%s, ue(v) : %u", what, *where);
}

static void HEVC_SWSR_SE(char *what, int *where, void *swsr_ctx)
{
	*where = swsr_read_signed_expgoulomb(swsr_ctx);
	pr_info("%s, se(v) : %u", what, *where);
}

static void HEVC_SWSR_FN(char *what, u8 *where, u8 numbits, u8 pattern,
...
static void HEVC_UCHECK(char *what, u32 val, u32 expected, enum bspp_error_type *bspperror)
{
	if (val != expected) {
		*bspperror |= BSPP_ERROR_INVALID_VALUE;
...
static void HEVC_RANGEUCHECK(char *what, u32 val, u32 min, u32 max, enum bspp_error_type *bspperror)
{
	if ((min > 0 && val < min) || val > max) {
		*bspperror |= BSPP_ERROR_INVALID_VALUE;
		pr_warn("Value of %s out out range (expected: [%u, %u], got: %u)",
			what, min, max, val);
	}
}

static void HEVC_RANGESCHECK(char *what, int val, int min, int max, enum bspp_error_type *bspperror)
{
	if (val < min || val > max) {
		*bspperror |= BSPP_ERROR_INVALID_VALUE;
		pr_warn("Value of %s out out range (expected: [%d, %d], got: %d)",
			what, min, max, val);
	}
}

#define HEVC_STATIC_ASSERT(expr) ((void)sizeof(char[1 - 2 * !(expr)]))
...
#define HEVC_MAX(a, b) ({ \
		...
		typeof(b) __b = b; \
		(((__a) >= (__b)) ? (__a) : (__b)); })
#define HEVC_ALIGN(_val, _alignment) ({ \
...
static enum bspp_error_type
bspp_hevc_parse_vps(void *sr_ctx, struct bspp_hevc_vps *vps);
...
static void bspp_hevc_parsehrdparams(void *sr_ctx,
				     struct bspp_hevc_hrd_parameters *hrdparams,
				     u8 common_infpresent,
				     u8 max_numsublayers_minus1);
...
static enum bspp_error_type
bspp_hevc_parse_slicesegmentheader(void *sr_ctx, void *str_res,
...
static enum bspp_error_type
bspp_hevc_parse_profiletierlevel(void *sr_ctx,
				 struct bspp_hevc_profile_tierlevel *ptl,
				 u8 vps_maxsublayers_minus1);

static void
bspp_hevc_getdefault_scalinglist(u8 size_id, u8 matrix_id,
				 const u8 **default_scalinglist,
				 unsigned int *size);

static enum bspp_error_type
bspp_hevc_parse_scalinglistdata(void *sr_ctx,
...
static enum bspp_error_type
bspp_hevc_parse_shortterm_refpicset(void *sr_ctx,
...
static u32 bspp_ceil_log2(u32 linear_val);

static u8 bspp_hevc_picture_is_irap(enum hevc_nalunittype  nalunit_type);

static u8 bspp_hevc_picture_is_cra(enum hevc_nalunittype  nalunit_type);

static u8 bspp_hevc_picture_is_idr(enum hevc_nalunittype  nalunit_type);

static u8 bspp_hevc_picture_is_bla(enum hevc_nalunittype  nalunit_type);

static u8 bspp_hevc_picture_getnorasl_outputflag(enum hevc_nalunittype nalunit_type,
						 struct bspp_hevc_inter_pict_ctx *inter_pict_ctx);

static u8 bspp_hevc_range_extensions_is_enabled(struct bspp_hevc_profile_tierlevel *profile_tierlevel);
...
static int bspp_hevc_unitparser(void *swsr_ctx, struct bspp_unit_data *unitdata)
{
	...
	u8 nal_unit_type = 0;
	u8 nuh_layer_id = 0;
	u8 nuh_temporal_id_plus1 = 0;

	HEVC_SWSR_FN("forbidden_zero_bit",
		     &forbidden_zero_bit, 1, 0, &parse_err, sr_ctx);
	HEVC_SWSR_UN("nal_unit_type", (u32 *)&nal_unit_type, 6, sr_ctx);
	/* for current version of HEVC nuh_layer_id "shall be equal to 0" */
	HEVC_SWSR_FN("nuh_layer_id", &nuh_layer_id, 6, 0, &parse_err, sr_ctx);
	HEVC_SWSR_UN("nuh_temporal_id_plus1", (u32 *)&nuh_temporal_id_plus1, 3, sr_ctx);
	...
	case BSPP_UNIT_SEQUENCE:
	{
		...
		fwsps =
		(struct hevcfw_sequence_ps *)((u8 *)tmp->ddbuf_info.cpu_virt_addr +
						tmp->buf_offset);
	...
	case BSPP_UNIT_PPS:
	{
		...
		/* handle firmware headers */
		fw_pps =
		(struct hevcfw_picture_ps *)((u8 *)tmp->ddbuf_info.cpu_virt_addr +
						tmp->buf_offset);
	...
	case BSPP_UNIT_PICTURE:
	{
		...
		parse_error =
		bspp_hevc_parse_slicesegmentheader(sr_ctx,
						   unitdata->str_res_handle,
						   &ssh,
						   nal_unit_type,
						   &vps_info,
		...
		if (ssh.first_slice_segment_in_pic_flag) {
			...
			bspp_hevc_fillpicturehdr(&sequ_hdr_info->sequ_hdr_info.com_sequ_hdr_info,
						 ...
						 (struct bspp_hevc_sps *)sequ_hdr_info->secure_sequence_info,
			...
			if (!inter_pict_ctx->hevc_ctx.sei_info_attached_to_pic) {
				...
				if (inter_pict_ctx->hevc_ctx.sei_rawdata_list) {
					...
					/* Otherwise expose a handle a picture header field to attach SEI list later. */
			...
			if (unitdata->new_closed_gop &&
			    bspp_hevc_picture_is_irap(nal_unit_type) &&
			    bspp_hevc_picture_getnorasl_outputflag(nal_unit_type,
								   &inter_pict_ctx->hevc_ctx)) {
				if (bspp_hevc_picture_is_cra(nal_unit_type))
...
static void bspp_hevc_finaliseparsing(void *str_alloc,
	...
	if (!parse_state->inter_pict_ctx->hevc_ctx.sei_info_attached_to_pic &&
		...
		if (parse_state->inter_pict_ctx->hevc_ctx.hndl_pichdr_sei_rawdata_list) {
			...
			*parse_state->inter_pict_ctx->hevc_ctx.hndl_pichdr_sei_rawdata_list =
			...
			bspp_freeraw_sei_datalist(str_alloc,
						  parse_state->inter_pict_ctx->hevc_ctx.sei_rawdata_list);
		}
	}
...
static enum bspp_error_type bspp_hevc_parse_vps(void *sr_ctx,
						struct bspp_hevc_vps *vps)
{
	u32 parse_err = BSPP_ERROR_NONE;
	u32 i, j;
	...
	HEVC_SWSR_UN("vps_video_parameter_set_id",
		     (u32 *)&vps->vps_video_parameter_set_id, 4, sr_ctx);
	HEVC_SWSR_UN("vps_reserved_three_2bits",
		     (u32 *)&vps->vps_reserved_three_2bits, 2, sr_ctx);
	HEVC_SWSR_UN("vps_max_layers_minus1",
		     (u32 *)&vps->vps_max_layers_minus1, 6, sr_ctx);
	HEVC_SWSR_UN("vps_max_sub_layers_minus1",
		     (u32 *)&vps->vps_max_sub_layers_minus1, 3, sr_ctx);
	HEVC_RANGEUCHECK("vps_max_sub_layers_minus1",
	...
	HEVC_SWSR_UN("vps_reserved_0xffff_16bits",
		     (u32 *)&vps->vps_reserved_0xffff_16bits, 16, sr_ctx);
	...
	parse_err |=
		bspp_hevc_parse_profiletierlevel(sr_ctx,
						 &vps->profiletierlevel,
	...
	for (i = vps->vps_sub_layer_ordering_info_present_flag ?
		...
		HEVC_SWSR_UE("vps_max_dec_pic_buffering_minus1",
			     (u32 *)&vps->vps_max_dec_pic_buffering_minus1[i], sr_ctx);
		HEVC_SWSR_UE("vps_max_num_reorder_pics",
			     (u32 *)&vps->vps_max_num_reorder_pics[i], sr_ctx);
		HEVC_SWSR_UE("vps_max_latency_increase_plus1",
			     (u32 *)&vps->vps_max_latency_increase_plus1[i], sr_ctx);
	}

	HEVC_SWSR_UN("vps_max_layer_id", (u32 *)&vps->vps_max_layer_id, 6, sr_ctx);
	HEVC_SWSR_UE("vps_num_layer_sets_minus1",
		     (u32 *)&vps->vps_num_layer_sets_minus1, sr_ctx);
	...
	HEVC_SWSR_U1("vps_timing_info_present_flag",
		     &vps->vps_timing_info_present_flag, sr_ctx);
	if (vps->vps_timing_info_present_flag) {
		HEVC_SWSR_UN("vps_num_units_in_tick",
			     (u32 *)&vps->vps_num_units_in_tick, 32, sr_ctx);
		HEVC_SWSR_UN("vps_time_scale",
			     (u32 *)&vps->vps_time_scale, 32, sr_ctx);
		...
		if (vps->vps_poc_proportional_to_timing_flag)
			HEVC_SWSR_UE("vps_num_ticks_poc_diff_one_minus1",
				     (u32 *)&vps->vps_num_ticks_poc_diff_one_minus1,
				     sr_ctx);

		HEVC_SWSR_UE("vps_num_hrd_parameters",
			     (u32 *)&vps->vps_num_hrd_parameters, sr_ctx);

		/* consume hrd_parameters */
		for (i = 0; i < vps->vps_num_hrd_parameters; i++) {
			unsigned short hrd_layer_set_idx;
			u8 cprms_present_flag = 1;
			struct bspp_hevc_hrd_parameters hrdparams;

			HEVC_SWSR_UE("hrd_layer_set_idx",
				     (u32 *)&hrd_layer_set_idx, sr_ctx);
			if (i > 0)
				HEVC_SWSR_U1("cprms_present_flag",
					     &cprms_present_flag, sr_ctx);
...
{
	u8 i;
	u8 cpb_cnt = hrdparams->cpb_cnt_minus1[sublayer_id];
	...
	for (i = 0; i <= cpb_cnt; i++) {
		HEVC_SWSR_UE("bit_rate_value_minus1",
			     (u32 *)&sublay_hrdparams->bit_rate_value_minus1[i], sr_ctx);
		HEVC_SWSR_UE("cpb_size_value_minus1",
			     (u32 *)&sublay_hrdparams->cpb_size_value_minus1[i], sr_ctx);
		if (hrdparams->sub_pic_hrd_params_present_flag) {
			HEVC_SWSR_UE("cpb_size_du_value_minus1",
				     (u32 *)&sublay_hrdparams->cpb_size_du_value_minus1[i],
				     sr_ctx);
			HEVC_SWSR_UE("bit_rate_du_value_minus1",
				     (u32 *)&sublay_hrdparams->bit_rate_du_value_minus1[i],
				     sr_ctx);
		}
		...
		if (hrdparams->nal_hrd_parameters_present_flag ||
			...
			if (hrdparams->sub_pic_hrd_params_present_flag) {
				HEVC_SWSR_UN("tick_divisor_minus2",
					     (u32 *)&hrdparams->tick_divisor_minus2,
					     8, sr_ctx);
				HEVC_SWSR_UN("du_cpb_removal_delay_increment_length_minus1",
					     (u32 *)&hrdparams->du_cpb_removal_delay_increment_length_minus1,
					     5, sr_ctx);
				...
				HEVC_SWSR_UN("dpb_output_delay_du_length_minus1",
					     (u32 *)&hrdparams->dpb_output_delay_du_length_minus1,
					     5, sr_ctx);
			}
			HEVC_SWSR_UN("bit_rate_scale",
				     (u32 *)&hrdparams->bit_rate_scale, 4, sr_ctx);
			HEVC_SWSR_UN("cpb_size_scale",
				     (u32 *)&hrdparams->cpb_size_scale, 4, sr_ctx);
			if (hrdparams->sub_pic_hrd_params_present_flag)
				HEVC_SWSR_UN("cpb_size_du_scale",
					     (u32 *)&hrdparams->cpb_size_du_scale,
					     4, sr_ctx);

			HEVC_SWSR_UN("initial_cpb_removal_delay_length_minus1",
				     (u32 *)&hrdparams->initial_cpb_removal_delay_length_minus1,
				     5, sr_ctx);
			HEVC_SWSR_UN("au_cpb_removal_delay_length_minus1",
				     (u32 *)&hrdparams->au_cpb_removal_delay_length_minus1,
				     5, sr_ctx);
			HEVC_SWSR_UN("dpb_output_delay_length_minus1",
				     (u32 *)&hrdparams->dpb_output_delay_length_minus1,
				     5, sr_ctx);
		}
	}
	for (i = 0; i <= max_numsublayers_minus1; i++) {
		...
		if (hrdparams->fixed_pic_rate_within_cvs_flag[i])
			HEVC_SWSR_UE("elemental_duration_in_tc_minus1",
				     (u32 *)&hrdparams->elemental_duration_in_tc_minus1[i],
				     sr_ctx);
		else
		...
		if (!hrdparams->low_delay_hrd_flag[i])
			HEVC_SWSR_UE("cpb_cnt_minus1",
				     (u32 *)&hrdparams->cpb_cnt_minus1[i], sr_ctx);
...
static enum bspp_error_type
bspp_hevc_parsevui_parameters(void *sr_ctx,
			      ...
			      u8 sps_max_sub_layers_minus1)
{
	...
	if (vui_params->aspect_ratio_info_present_flag) {
		HEVC_SWSR_UN("aspect_ratio_idc",
			     (u32 *)&vui_params->aspect_ratio_idc, 8, sr_ctx);
		if (vui_params->aspect_ratio_idc == HEVC_EXTENDED_SAR) {
			HEVC_SWSR_UN("sar_width",
				     (u32 *)&vui_params->sar_width, 16, sr_ctx);
			HEVC_SWSR_UN("sar_height",
				     (u32 *)&vui_params->sar_height, 16, sr_ctx);
		}
	}
	...
	if (vui_params->video_signal_type_present_flag) {
		HEVC_SWSR_UN("video_format",
			     (u32 *)&vui_params->video_format, 3, sr_ctx);
		...
		if (vui_params->colour_description_present_flag) {
			HEVC_SWSR_UN("colour_primaries",
				     (u32 *)&vui_params->colour_primaries, 8, sr_ctx);
			HEVC_SWSR_UN("transfer_characteristics",
				     (u32 *)&vui_params->transfer_characteristics,
				     8, sr_ctx);
			HEVC_SWSR_UN("matrix_coeffs",
				     (u32 *)&vui_params->matrix_coeffs, 8, sr_ctx);
		}
	}
	...
	if (vui_params->chroma_loc_info_present_flag) {
		HEVC_SWSR_UE("chroma_sample_loc_type_top_field",
			     (u32 *)&vui_params->chroma_sample_loc_type_top_field,
			     sr_ctx);
		...
		HEVC_SWSR_UE("chroma_sample_loc_type_bottom_field",
			     (u32 *)&vui_params->chroma_sample_loc_type_bottom_field,
			     sr_ctx);
	...
	if (vui_params->default_display_window_flag) {
		HEVC_SWSR_UE("def_disp_win_left_offset",
			     (u32 *)&vui_params->def_disp_win_left_offset, sr_ctx);
		HEVC_SWSR_UE("def_disp_win_right_offset",
			     (u32 *)&vui_params->def_disp_win_right_offset, sr_ctx);
		HEVC_SWSR_UE("def_disp_win_top_offset",
			     (u32 *)&vui_params->def_disp_win_top_offset, sr_ctx);
		HEVC_SWSR_UE("def_disp_win_bottom_offset",
			     (u32 *)&vui_params->def_disp_win_bottom_offset, sr_ctx);
	}
	...
	if (vui_params->vui_timing_info_present_flag) {
		HEVC_SWSR_UN("vui_num_units_in_tick",
			     (u32 *)&vui_params->vui_num_units_in_tick, 32, sr_ctx);
		HEVC_SWSR_UN("vui_time_scale",
			     (u32 *)&vui_params->vui_time_scale, 32, sr_ctx);
		...
		if (vui_params->vui_poc_proportional_to_timing_flag)
			HEVC_SWSR_UE("vui_num_ticks_poc_diff_one_minus1",
				     (u32 *)&vui_params->vui_num_ticks_poc_diff_one_minus1,
				     sr_ctx);
		...
		if (vui_params->vui_hrd_parameters_present_flag)
			bspp_hevc_parsehrdparams(sr_ctx,
						 &vui_params->vui_hrd_params,
	...
	if (vui_params->bitstream_restriction_flag) {
		...
		HEVC_SWSR_UE("min_spatial_segmentation_idc",
			     (u32 *)&vui_params->min_spatial_segmentation_idc, sr_ctx);
		HEVC_RANGEUCHECK("min_spatial_segmentation_idc",
				 vui_params->min_spatial_segmentation_idc,
				 0, 4095, &parse_err);

		HEVC_SWSR_UE("max_bytes_per_pic_denom",
			     (u32 *)&vui_params->max_bytes_per_pic_denom, sr_ctx);
		HEVC_RANGEUCHECK("max_bytes_per_pic_denom",
				 vui_params->max_bytes_per_pic_denom,
				 0, 16, &parse_err);

		HEVC_SWSR_UE("max_bits_per_min_cu_denom",
			     (u32 *)&vui_params->max_bits_per_min_cu_denom, sr_ctx);
		HEVC_RANGEUCHECK("max_bits_per_min_cu_denom",
				 vui_params->max_bits_per_min_cu_denom,
				 0, 16, &parse_err);

		HEVC_SWSR_UE("log2_max_mv_length_horizontal",
			     (u32 *)&vui_params->log2_max_mv_length_horizontal, sr_ctx);
		...
		HEVC_SWSR_UE("log2_max_mv_length_vertical",
			     (u32 *)&vui_params->log2_max_mv_length_vertical, sr_ctx);
...
static enum bspp_error_type
bspp_hevc_parse_spsrange_extensions(void *sr_ctx,
					       ...
					       struct bspp_hevc_sps *sps)
{
	...
	HEVC_SWSR_UN("sps_video_parameter_set_id",
		     (u32 *)&sps->sps_video_parameter_set_id, 4, sr_ctx);
	HEVC_SWSR_UN("sps_max_sub_layers_minus1",
		     (u32 *)&sps->sps_max_sub_layers_minus1, 3, sr_ctx);
	HEVC_RANGEUCHECK("sps_max_sub_layers_minus1",
	...
	parse_err |= bspp_hevc_parse_profiletierlevel(sr_ctx,
						      &sps->profile_tier_level,
						      sps->sps_max_sub_layers_minus1);

	HEVC_SWSR_UE("sps_seq_parameter_set_id",
		     (u32 *)&sps->sps_seq_parameter_set_id, sr_ctx);
	HEVC_RANGEUCHECK("sps_seq_parameter_set_id",
			 sps->sps_seq_parameter_set_id, 0,
			 HEVC_MAX_SPS_COUNT - 1, &parse_err);

	HEVC_SWSR_UE("chroma_format_idc", (u32 *)&sps->chroma_format_idc, sr_ctx);
	HEVC_RANGEUCHECK("chroma_format_idc",
			 sps->chroma_format_idc, 0, 3, &parse_err);
	...
	HEVC_SWSR_UE("pic_width_in_luma_samples",
		     (u32 *)&sps->pic_width_in_luma_samples, sr_ctx);
	HEVC_SWSR_UE("pic_height_in_luma_samples",
		     (u32 *)&sps->pic_height_in_luma_samples, sr_ctx);

	HEVC_SWSR_U1("conformance_window_flag",
		     &sps->conformance_window_flag, sr_ctx);
	...
	if (sps->conformance_window_flag) {
		HEVC_SWSR_UE("conf_win_left_offset",
			     (u32 *)&sps->conf_win_left_offset, sr_ctx);
		HEVC_SWSR_UE("conf_win_right_offset",
			     (u32 *)&sps->conf_win_right_offset, sr_ctx);
		HEVC_SWSR_UE("conf_win_top_offset",
			     (u32 *)&sps->conf_win_top_offset, sr_ctx);
		HEVC_SWSR_UE("conf_win_bottom_offset",
			     (u32 *)&sps->conf_win_bottom_offset, sr_ctx);
	}

	HEVC_SWSR_UE("bit_depth_luma_minus8",
		     (u32 *)&sps->bit_depth_luma_minus8, sr_ctx);
	HEVC_RANGEUCHECK("bit_depth_luma_minus8",
			 sps->bit_depth_luma_minus8, 0, 6, &parse_err);
	HEVC_SWSR_UE("bit_depth_chroma_minus8",
		     (u32 *)&sps->bit_depth_chroma_minus8, sr_ctx);
	HEVC_RANGEUCHECK("bit_depth_chroma_minus8",
			 sps->bit_depth_chroma_minus8, 0, 6, &parse_err);

	HEVC_SWSR_UE("log2_max_pic_order_cnt_lsb_minus4",
		     (u32 *)&sps->log2_max_pic_order_cnt_lsb_minus4, sr_ctx);
	...
	for (i = (sps->sps_sub_layer_ordering_info_present_flag ?
		...
		HEVC_SWSR_UE("sps_max_dec_pic_buffering_minus1",
			     (u32 *)&sps->sps_max_dec_pic_buffering_minus1[i], sr_ctx);
		HEVC_SWSR_UE("sps_max_num_reorder_pics",
			     (u32 *)&sps->sps_max_num_reorder_pics[i], sr_ctx);
		HEVC_SWSR_UE("sps_max_latency_increase_plus1",
			     (u32 *)&sps->sps_max_latency_increase_plus1[i], sr_ctx);
	}

	HEVC_SWSR_UE("log2_min_luma_coding_block_size_minus3",
		     (u32 *)&sps->log2_min_luma_coding_block_size_minus3, sr_ctx);
	HEVC_SWSR_UE("log2_diff_max_min_luma_coding_block_size",
		     (u32 *)&sps->log2_diff_max_min_luma_coding_block_size, sr_ctx);
	HEVC_SWSR_UE("log2_min_transform_block_size_minus2",
		     (u32 *)&sps->log2_min_transform_block_size_minus2, sr_ctx);
	HEVC_SWSR_UE("log2_diff_max_min_transform_block_size",
		     (u32 *)&sps->log2_diff_max_min_transform_block_size, sr_ctx);
	HEVC_SWSR_UE("max_transform_hierarchy_depth_inter",
		     (u32 *)&sps->max_transform_hierarchy_depth_inter, sr_ctx);
	HEVC_SWSR_UE("max_transform_hierarchy_depth_intra",
		     (u32 *)&sps->max_transform_hierarchy_depth_intra, sr_ctx);

	HEVC_SWSR_U1("scaling_list_enabled_flag",
		     &sps->scaling_list_enabled_flag, sr_ctx);
	...
	if (sps->scaling_list_enabled_flag) {
		...
		if (sps->sps_scaling_list_data_present_flag)
			parse_err |=
			bspp_hevc_parse_scalinglistdata(sr_ctx,
	...
	if (sps->pcm_enabled_flag) {
		HEVC_SWSR_UN("pcm_sample_bit_depth_luma_minus1",
			     (u32 *)&sps->pcm_sample_bit_depth_luma_minus1,
			     4, sr_ctx);
		HEVC_SWSR_UN("pcm_sample_bit_depth_chroma_minus1",
			     (u32 *)&sps->pcm_sample_bit_depth_chroma_minus1,
			     4, sr_ctx);
		HEVC_SWSR_UE("log2_min_pcm_luma_coding_block_size_minus3",
			     (u32 *)&sps->log2_min_pcm_luma_coding_block_size_minus3,
			     sr_ctx);
		HEVC_SWSR_UE("log2_diff_max_min_pcm_luma_coding_block_size",
			     (u32 *)&sps->log2_diff_max_min_pcm_luma_coding_block_size,
			     sr_ctx);
	...
	HEVC_SWSR_UE("num_short_term_ref_pic_sets",
		     (u32 *)&sps->num_short_term_ref_pic_sets, sr_ctx);
	HEVC_RANGEUCHECK("num_short_term_ref_pic_sets",
	...
	if (sps->long_term_ref_pics_present_flag) {
		HEVC_SWSR_UE("num_long_term_ref_pics_sps",
			     (u32 *)&sps->num_long_term_ref_pics_sps, sr_ctx);
		...
		for (i = 0; i < sps->num_long_term_ref_pics_sps; ++i) {
			HEVC_SWSR_UN("lt_ref_pic_poc_lsb_sps",
				     (u32 *)&sps->lt_ref_pic_poc_lsb_sps[i],
	...
	HEVC_SWSR_U1("sps_temporal_mvp_enabled_flag",
		     &sps->sps_temporal_mvp_enabled_flag, sr_ctx);
	HEVC_SWSR_U1("strong_intra_smoothing_enabled_flag",
		     &sps->strong_intra_smoothing_enabled_flag, sr_ctx);
	HEVC_SWSR_U1("vui_parameters_present_flag",
		     &sps->vui_parameters_present_flag, sr_ctx);

	if (sps->vui_parameters_present_flag)
		bspp_hevc_parsevui_parameters(sr_ctx,
					      &sps->vui_params,
					      sps->sps_max_sub_layers_minus1);

	HEVC_SWSR_U1("sps_extension_present_flag",
		     &sps->sps_extension_present_flag, sr_ctx);
	if (sps->sps_extension_present_flag &&
	    bspp_hevc_range_extensions_is_enabled(&sps->profile_tier_level)) {
		HEVC_SWSR_U1("sps_range_extensions_flag",
			     &sps->sps_range_extensions_flag, sr_ctx);

		HEVC_SWSR_UN("sps_extension_7bits",
			     (u32 *)&sps->sps_extension_7bits, 7, sr_ctx);
		/*
		 *  ignore extension data. Although we inform
		 * if some non-zero data was found
		 */
		HEVC_UCHECK("sps_extension_7bits",
			    sps->sps_extension_7bits, 0, &parse_err);
		/*
		...
		if (sps->sps_range_extensions_flag)
			parse_err |=
			bspp_hevc_parse_spsrange_extensions(sr_ctx,
							    &sps->range_exts);
	}
	/*
	...
	for (i = 0; i <= sps->sps_max_sub_layers_minus1; ++i) {
		if (sps->sps_max_latency_increase_plus1) {
...
static int bspp_hevc_release_sequhdrinfo(void *str_alloc, void *secure_spsinfo)
{
	...
	bspp_streamrelese_rawbstrdataplain(str_alloc,
					   (void *)hevc_sps->vui_raw_data);
	return 0;
}
...
static enum bspp_error_type
bspp_hevc_parsepps_range_extensions(void *sr_ctx,
				    ...
				    u8 log2_diff_max_min_luma_coding_block_size)
{
	...
	if (transform_skip_enabled_flag)
		HEVC_SWSR_UE("log2_max_transform_skip_block_size_minus2",
			     (u32 *)&range_exts->log2_max_transform_skip_block_size_minus2,
			     sr_ctx);
	...
	if (range_exts->chroma_qp_offset_list_enabled_flag) {
		...
		HEVC_SWSR_UE("diff_cu_chroma_qp_offset_depth",
			     (u32 *)&range_exts->diff_cu_chroma_qp_offset_depth,
			     sr_ctx);
		...
		HEVC_SWSR_UE("chroma_qp_offset_list_len_minus1",
			     (u32 *)&range_exts->chroma_qp_offset_list_len_minus1,
			     sr_ctx);
		...
		for (i = 0; i <= range_exts->chroma_qp_offset_list_len_minus1;
		     i++) {
			HEVC_SWSR_SE("cb_qp_offset_list",
				     (int *)&range_exts->cb_qp_offset_list[i], sr_ctx);
			HEVC_RANGESCHECK("cb_qp_offset_list",
					 range_exts->cb_qp_offset_list[i],
					 -12, 12, &parse_err);
			HEVC_SWSR_SE("cr_qp_offset_list",
				     (int *)&range_exts->cr_qp_offset_list[i], sr_ctx);
			HEVC_RANGESCHECK("cr_qp_offset_list",
					 range_exts->cr_qp_offset_list[i],
					 -12, 12, &parse_err);
		}
	}
	HEVC_SWSR_UE("log2_sao_offset_scale_luma",
		     (u32 *)&range_exts->log2_sao_offset_scale_luma, sr_ctx);
	...
	HEVC_SWSR_UE("log2_sao_offset_scale_chroma",
		     (u32 *)&range_exts->log2_sao_offset_scale_chroma, sr_ctx);
...
static u8
bspp_hevc_checkppsrangeextensions(struct bspp_hevc_pps_range_exts *range_exts)
{
	VDEC_ASSERT(range_exts);
...
static enum bspp_error_type
bspp_hevc_parsepps(void *sr_ctx, void *str_res, struct bspp_hevc_pps *pps)
{
	enum bspp_error_type  parse_err = BSPP_ERROR_NONE;
	...
	HEVC_SWSR_UE("pps_pic_parameter_set_id",
		     (u32 *)&pps->pps_pic_parameter_set_id, sr_ctx);
	HEVC_RANGEUCHECK("pps_pic_parameter_set_id",
			 pps->pps_pic_parameter_set_id, 0,
			 HEVC_MAX_PPS_COUNT - 1, &parse_err);
	HEVC_SWSR_UE("pps_seq_parameter_set_id",
		     (u32 *)&pps->pps_seq_parameter_set_id, sr_ctx);
	HEVC_RANGEUCHECK("pps_seq_parameter_set_id",
	...
	HEVC_SWSR_UN("num_extra_slice_header_bits",
		     (u32 *)&pps->num_extra_slice_header_bits, 3, sr_ctx);
	HEVC_SWSR_U1("sign_data_hiding_enabled_flag",
		     &pps->sign_data_hiding_enabled_flag, sr_ctx);
	HEVC_SWSR_U1("cabac_init_present_flag",
		     &pps->cabac_init_present_flag, sr_ctx);
	HEVC_SWSR_UE("num_ref_idx_l0_default_active_minus1",
		     (u32 *)&pps->num_ref_idx_l0_default_active_minus1, sr_ctx);
	...
	HEVC_SWSR_UE("num_ref_idx_l1_default_active_minus1",
		     (u32 *)&pps->num_ref_idx_l1_default_active_minus1, sr_ctx);
	HEVC_RANGEUCHECK("num_ref_idx_l1_default_active_minus1",
			 pps->num_ref_idx_l1_default_active_minus1, 0, 14,
			 &parse_err);
	HEVC_SWSR_SE("init_qp_minus26", (int *)&pps->init_qp_minus26, sr_ctx);

	if (sps)
		HEVC_RANGESCHECK("init_qp_minus26", pps->init_qp_minus26,
				 -(26 + (6 * sps->bit_depth_luma_minus8)), 25,
				 &parse_err);

	HEVC_SWSR_U1("constrained_intra_pred_flag",
		     &pps->constrained_intra_pred_flag, sr_ctx);
	HEVC_SWSR_U1("transform_skip_enabled_flag",
		     &pps->transform_skip_enabled_flag, sr_ctx);

	HEVC_SWSR_U1("cu_qp_delta_enabled_flag",
		     &pps->cu_qp_delta_enabled_flag, sr_ctx);

	if (pps->cu_qp_delta_enabled_flag)
		HEVC_SWSR_UE("diff_cu_qp_delta_depth",
			     (u32 *)&pps->diff_cu_qp_delta_depth, sr_ctx);

	HEVC_SWSR_SE("pps_cb_qp_offset", (int *)&pps->pps_cb_qp_offset, sr_ctx);
	HEVC_RANGESCHECK("pps_cb_qp_offset", pps->pps_cb_qp_offset,
			 -12, 12, &parse_err);
	HEVC_SWSR_SE("pps_cr_qp_offset", (int *)&pps->pps_cr_qp_offset, sr_ctx);
	HEVC_RANGESCHECK("pps_cr_qp_offset", pps->pps_cr_qp_offset,
			 -12, 12, &parse_err);
	...
	if (pps->tiles_enabled_flag) {
		HEVC_SWSR_UE("num_tile_columns_minus1",
			     (u32 *)&pps->num_tile_columns_minus1, sr_ctx);
		HEVC_RANGEUCHECK("num_tile_columns_minus1",
		...
		HEVC_SWSR_UE("num_tile_rows_minus1", (u32 *)&pps->num_tile_rows_minus1,
			     &sr_ctx);
		HEVC_RANGEUCHECK("num_tile_rows_minus1",
		...
		HEVC_SWSR_U1("uniform_spacing_flag",
			     &pps->uniform_spacing_flag, sr_ctx);
		...
		if (!pps->uniform_spacing_flag) {
			...
			for (i = 0; i < pps->num_tile_columns_minus1; ++i)
				HEVC_SWSR_UE("column_width_minus1",
					     (u32 *)&pps->column_width_minus1[i],
					     sr_ctx);

			for (i = 0; i < pps->num_tile_rows_minus1; ++i)
				HEVC_SWSR_UE("row_height_minus1",
					     (u32 *)&pps->row_height_minus1[i],
					     sr_ctx);
		}
	...
	if (pps->deblocking_filter_control_present_flag) {
		...
		if (!pps->pps_deblocking_filter_disabled_flag) {
			HEVC_SWSR_SE("pps_beta_offset_div2",
				     (int *)&pps->pps_beta_offset_div2, sr_ctx);
			HEVC_RANGESCHECK("pps_beta_offset_div2",
					 pps->pps_beta_offset_div2, -6, 6, &parse_err);
			HEVC_SWSR_SE("pps_tc_offset_div2",
				     (int *)&pps->pps_tc_offset_div2, sr_ctx);
			HEVC_RANGESCHECK("pps_tc_offset_div2",
					 pps->pps_tc_offset_div2, -6, 6, &parse_err);
		}
	}
	...
	HEVC_SWSR_UE("log2_parallel_merge_level_minus2",
		     (u32 *)&pps->log2_parallel_merge_level_minus2, sr_ctx);
	HEVC_SWSR_U1("slice_segment_header_extension_present_flag",
		     &pps->slice_segment_header_extension_present_flag, sr_ctx);

	HEVC_SWSR_U1("pps_extension_present_flag",
		     &pps->pps_extension_present_flag, sr_ctx);
	if (pps->pps_extension_present_flag &&
	    bspp_hevc_range_extensions_is_enabled(&sps->profile_tier_level)) {
		HEVC_SWSR_U1("pps_range_extensions_flag",
			     &pps->pps_range_extensions_flag, sr_ctx);
		HEVC_SWSR_UN("pps_extension_7bits",
			     (u32 *)&pps->pps_extension_7bits, 7, sr_ctx);
		/*
		 * ignore extension data. Although we inform
		 * if some non-zero data was found
		 */
		HEVC_UCHECK("pps_extension_7bits",
			    pps->pps_extension_7bits, 0, &parse_err);
...
static void bspp_hevc_dotilecalculations(struct bspp_hevc_sps *sps,
					 struct bspp_hevc_pps *pps)
{
	u16 colwidth[HEVC_MAX_TILE_COLS];
	u16 rowheight[HEVC_MAX_TILE_ROWS];
	u8 i;
	...
	if (pps->uniform_spacing_flag) {
		...
		if (rowheight[pps->num_tile_rows_minus1] >
				   ...
				   struct bspp_pps_info **ppsinfo)
{
	...
	if (bspp_hevc_picture_is_irap(nalunit_type))
		HEVC_SWSR_U1("no_output_of_prior_pics_flag",
			     &ssh->no_output_of_prior_pics_flag, sr_ctx);

	HEVC_SWSR_UE("slice_pic_parameter_set_id",
		     (u32 *)&ssh->slice_pic_parameter_set_id, sr_ctx);
	HEVC_RANGEUCHECK("slice_pic_parameter_set_id",
	...
	*ppsinfo = bspp_get_pps_hdr(str_res, ssh->slice_pic_parameter_set_id);
	...
	VDEC_ASSERT(pps->pps_pic_parameter_set_id ==
		    ssh->slice_pic_parameter_set_id);
	...
	*vpsinfo = bspp_get_vpshdr(str_res, sps->sps_video_parameter_set_id);
	...
	VDEC_ASSERT(vps->vps_video_parameter_set_id ==
				 ...
				 u8 vps_maxsublayers_minus1)
{
	...
	HEVC_SWSR_UN("general_profile_space",
		     (u32 *)&ptl->general_profile_space, 2, sr_ctx);
	HEVC_SWSR_U1("general_tier_flag",
		     &ptl->general_tier_flag, sr_ctx);
	HEVC_SWSR_UN("general_profile_idc",
		     (u32 *)&ptl->general_profile_idc, 5, sr_ctx);
	...
	if (ptl->general_profile_idc == 4 ||
		...
		HEVC_SWSR_UN("general_reserved_zero_44bits (1)",
			     &res, 32, sr_ctx);
		HEVC_UCHECK("general_reserved_zero_44bits (1)",
			    res, 0, &parse_err);
		HEVC_SWSR_UN("general_reserved_zero_44bits (2)",
			     &res, 12, sr_ctx);
		HEVC_UCHECK("general_reserved_zero_44bits (2)",
			    res, 0, &parse_err);
	}

	HEVC_SWSR_UN("general_level_idc", (u32 *)&ptl->general_level_idc, 8, sr_ctx);
	...
	for (i = 0; i < vps_maxsublayers_minus1; ++i) {
		if (ptl->sub_layer_profile_present_flag[i]) {
			HEVC_SWSR_UN("sub_layer_profile_space",
				     (u32 *)&ptl->sub_layer_profile_space[i], 2,
				     sr_ctx);
			HEVC_SWSR_U1("sub_layer_tier_flag",
				     &ptl->sub_layer_tier_flag[i], sr_ctx);
			HEVC_SWSR_UN("sub_layer_profile_idc",
				     (u32 *)&ptl->sub_layer_profile_idc[i], 5, sr_ctx);
			for (j = 0; j < HEVC_MAX_NUM_PROFILE_IDC; ++j)
		...
		if (ptl->sub_layer_level_present_flag[i])
			HEVC_SWSR_UN("sub_layer_level_idc",
				     (u32 *)&ptl->sub_layer_level_idc[i], 8, sr_ctx);
	}
	return parse_err;
}
...
static const u8 def_4x4[HEVC_SCALING_LIST_0_SIZE] = {
	16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16
};

static const u8 def_8x8_intra[HEVC_SCALING_LIST_123_SIZE] = {
...
static const u8 def_8x8_inter[HEVC_SCALING_LIST_123_SIZE] = {
 ...
 * to PVDEC order
 */
static const u8 HEVC_INV_ZZ_SCAN4[HEVC_SCALING_LIST_MATRIX_SIZE / 4]  = {
	0,  1,  2,  4,  3,  6,  7, 10,  5,  8,  9, 12, 11, 13, 14, 15
};

static const u8 HEVC_INV_ZZ_SCAN8[HEVC_SCALING_LIST_MATRIX_SIZE] = {
				...
				struct bspp_hevc_scalinglist_data *scaling_listdata)
{
	...
	for (size_id = 0; size_id < HEVC_SCALING_LIST_NUM_SIZES; ++size_id) {
		for (matrix_id = 0; matrix_id < ((size_id == 3) ? 2 : 6);
			...
			u8 scaling_list_pred_mode_flag = 0;

			HEVC_SWSR_U1("scaling_list_pred_mode_flag",
				     &scaling_list_pred_mode_flag, sr_ctx);
			if (!scaling_list_pred_mode_flag) {
				u8 scaling_list_pred_matrix_id_delta = 0;
				const u8 *defaultlist = NULL;
				unsigned int listsize = 0;

				HEVC_SWSR_UE("scaling_list_pred_matrixid_delta",
					     (u32 *)&scaling_list_pred_matrix_id_delta,
					     sr_ctx);
				...
				if (scaling_list_pred_matrix_id_delta == 0) {
					...
					if (size_id > 1)
						scaling_listdata->dccoeffs[size_id - 2][matrix_id] = 8 + 8;
				} else {
					u8 ref_matrix_id =
					...
					if (size_id > 1)
						...
						scaling_listdata->dccoeffs[size_id - 2][ref_matrix_id];
				}
			} else {
				/*
				...
				static const u8 matrix_max_coef_num = 64;
...
bspp_hevc_usedefault_scalinglists(struct bspp_hevc_scalinglist_data *scaling_listdata)
{
	...
	memset(scaling_listdata->dccoeffs,
	       8 + 8,
	       sizeof(scaling_listdata->dccoeffs));
}
				    ...
				    u8 in_slice_header)
{
	...
	if (inter_ref_pic_set_prediction_flag) {
		...
		int delta_rps = 0;
		u8 i = 0;
		u8 delta_idx_minus1 = 0;
		u8 delta_rps_sign = 0;
		unsigned short abs_delta_rps_minus1 = 0;
		u8 used_by_curr_pic_flag[HEVC_MAX_NUM_REF_PICS];
		u8 use_delta_flag[HEVC_MAX_NUM_REF_PICS];

		struct bspp_hevc_shortterm_refpicset *ref_strps = NULL;

		if (in_slice_header) {
			HEVC_SWSR_UE("delta_idx_minus1",
				     (u32 *)&delta_idx_minus1, sr_ctx);
			HEVC_RANGEUCHECK("delta_idx_minus1",
					 delta_idx_minus1, 0, st_rps_idx - 1,
					 &parse_err);
		}

		HEVC_SWSR_U1("delta_rps_sign", &delta_rps_sign, sr_ctx);
		HEVC_SWSR_UE("abs_delta_rps_minus1",
			     (u32 *)&abs_delta_rps_minus1, sr_ctx);
		HEVC_RANGEUCHECK("abs_delta_rps_minus1",
				 abs_delta_rps_minus1, 0, ((1 << 15) - 1),
				 &parse_err);
		...
		for (j_8 = 0; j_8 <= ref_strps->num_delta_pocs; ++j_8) {
			HEVC_SWSR_U1("used_by_curr_pic_flag",
				     &used_by_curr_pic_flag[j_8], sr_ctx);
			if (!used_by_curr_pic_flag[j_8])
				HEVC_SWSR_U1("use_delta_flag",
					     &use_delta_flag[j_8], sr_ctx);
		}
		...
		if (delta_rps < 0 &&
		    use_delta_flag[ref_strps->num_delta_pocs]) {
		...
		for (j_8 = 0; j_8 < ref_strps->num_positive_pics; ++j_8) {
			...
			if (dpoc > 0 &&
			    use_delta_flag[ref_strps->num_negative_pics + j_8]) {
				strps->delta_poc_s1[i] = dpoc;
				strps->used_bycurr_pic_s1[i++] =
		...
		u8 num_negative_pics = 0;
		u8 num_positive_pics = 0;
		unsigned short delta_poc_s0_minus1[HEVC_MAX_NUM_REF_PICS];
		u8 used_by_curr_pic_s0_flag[HEVC_MAX_NUM_REF_PICS];
		unsigned short delta_poc_s1_minus1[HEVC_MAX_NUM_REF_PICS];
		u8 used_by_curr_pic_s1_flag[HEVC_MAX_NUM_REF_PICS];
		u8 j = 0;

		HEVC_SWSR_UE("num_negative_pics", (u32 *)&num_negative_pics, sr_ctx);
		if (num_negative_pics > HEVC_MAX_NUM_REF_PICS) {
			num_negative_pics = HEVC_MAX_NUM_REF_PICS;
			parse_err |= BSPP_ERROR_CORRECTION_VALIDVALUE;
		}
		HEVC_SWSR_UE("num_positive_pics", (u32 *)&num_positive_pics, sr_ctx);
		...
		for (j = 0; j < num_negative_pics; ++j) {
			HEVC_SWSR_UE("delta_poc_s0_minus1",
				     (u32 *)&delta_poc_s0_minus1[j], sr_ctx);
			HEVC_RANGEUCHECK("delta_poc_s0_minus1",
					 delta_poc_s0_minus1[j], 0,
					 ((1 << 15) - 1), &parse_err);
			...
			if (j == 0)
				...
				strps->delta_poc_s0[j] =
						strps->delta_poc_s0[j - 1] -
						(delta_poc_s0_minus1[j] + 1);

			strps->used_bycurr_pic_s0[j] =
					used_by_curr_pic_s0_flag[j];
		}

		for (j = 0; j < num_positive_pics; j++) {
			HEVC_SWSR_UE("delta_poc_s1_minus1",
				     (u32 *)&delta_poc_s1_minus1[j], sr_ctx);
			HEVC_RANGEUCHECK("delta_poc_s1_minus1",
					 delta_poc_s1_minus1[j], 0,
					 ((1 << 15) - 1), &parse_err);
			...
			if (j == 0)
				...
				strps->delta_poc_s1[j] =
						...
						(delta_poc_s1_minus1[j] + 1);
			strps->used_bycurr_pic_s1[j] =
					used_by_curr_pic_s1_flag[j];
		}
	...
	BSPP_HEVC_SYNTAX("strps[%u]: num_delta_pocs: %u (%u (num_negative_pics) + %u (num_positive_pics))",
			 st_rps_idx,
			 strps->num_delta_pocs,
				       ...
				       struct vdec_comsequ_hdrinfo *common_seq)
{
	...
	u8 chroma_idc = sps->chroma_format_idc;
	struct pixel_pixinfo *pixel_info = &common_seq->pixel_info;
	u32 maxsub_layersmin1;
	u32 maxdpb_size;
	...
	pixel_info->pixfmt =
	pixel_get_pixfmt(pixel_info->chroma_fmt_idc,
			 pixel_info->chroma_interleave,
			 pixel_info->mem_pkg,
	...
	common_seq->max_frame_size.width =
			sps->pic_width_in_ctbs_y * sps->ctb_size_y;
	common_seq->max_frame_size.height =
			sps->pic_height_in_ctbs_y * sps->ctb_size_y;
	...
	HEVC_MAX(sps->sps_max_dec_pic_buffering_minus1[maxsub_layersmin1] + 1,
		 sps->sps_max_num_reorder_pics[maxsub_layersmin1]);

	if (sps->sps_max_latency_increase_plus1[maxsub_layersmin1]) {
		maxdpb_size =
		HEVC_MAX(maxdpb_size,
			 sps->sps_max_latency_pictures[maxsub_layersmin1]);
	}

	maxdpb_size = HEVC_MIN(maxdpb_size,
			       HEVC_MAX_NUM_REF_IDX_ACTIVE + 1);

	common_seq->min_pict_buf_num = HEVC_MAX(maxdpb_size, 6);
				     ...
				     struct bspp_hevc_vps *vps)
{
	...
	HEVC_ALIGN(sps->pic_width_in_luma_samples, HEVC_MIN_CODED_UNIT_SIZE);
	picture_hdr->coded_frame_size.height =
	HEVC_ALIGN(sps->pic_height_in_luma_samples, HEVC_MIN_CODED_UNIT_SIZE);
	picture_hdr->disp_info.enc_disp_region =
			common_seq->orig_display_region;
	...
	memset(picture_hdr->disp_info.pan_scan_windows,
	       0,
				 ...
				 struct hevcfw_sequence_ps *fwsps)
{
	...
	memcpy(fwsps->sps_max_dec_pic_buffering_minus1,
	...
	memcpy(fwsps->sps_max_num_reorder_pics,
	...
	memcpy(fwsps->sps_max_latency_increase_plus1,
	...
	memcpy(fwsps->used_by_curr_pic_lt_sps_flag,
	       ...
	       sizeof(fwsps->used_by_curr_pic_lt_sps_flag[0]) *
	       sps->num_long_term_ref_pics_sps);

	for (i = 0; i < sps->num_short_term_ref_pic_sets; ++i)
		bspp_hevc_fill_fwst_rps(&sps->rps_list[i],
					&fwsps->st_rps_list[i]);
	...
	memcpy(fwsps->sps_max_latency_pictures,
	       sps->sps_max_latency_pictures,
				    ...
				    struct hevcfw_short_term_ref_picset *fwstrps)
{
	...
	memcpy(fwstrps->used_bycurr_pic_s0, strps->used_bycurr_pic_s0,
	       sizeof(fwstrps->used_bycurr_pic_s0[0]) *
	       strps->num_negative_pics);
	...
	memcpy(fwstrps->used_bycurr_pic_s1, strps->used_bycurr_pic_s1,
	       sizeof(fwstrps->used_bycurr_pic_s1[0]) *
	       strps->num_positive_pics);
}
...
static void bspp_hevc_parse_codec_config(void *hndl_swsr_ctx, u32 *unit_count,
					 u32 *unit_array_count, u32 *delim_length,
					 u32 *size_delim_length)
{
	u64 value = 23;
...
static void bspp_hevc_update_unitcounts(void *hndl_swsr_ctx, u32 *unit_count,
					u32 *unit_array_count)
{
	if (*unit_array_count != 0) {
		...
		if (*unit_count == 0) {
			 ...
			 * without emulation prevention detection.
			 */
			swsr_consume_delim(hndl_swsr_ctx, SWSR_EMPREVENT_NONE,
					   0, &value);
				  ...
				  enum bspp_unit_type *bspp_unittype)
{
	...
	u8 type = (bitstream_unittype >> 1) & 0x3f;
