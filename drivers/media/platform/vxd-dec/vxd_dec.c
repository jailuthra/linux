...
static struct vxd_buffer *find_buffer(u32 buf_map_id, struct list_head *head)
{
...
static void return_worker(struct work_struct *work)
{
	...
	case VXD_CB_STRUNIT_PROCESSED:
		...
		buf = find_buffer(res->buf_map_id, &ctx->out_buffers);
		if (!buf) {
			dev_err(dev,
	...
	case VXD_CB_PICT_RELEASE:
		mutex_lock(&ctx->mutex);
		buf = find_buffer(res->buf_map_id, &ctx->reuse_queue);
		if (buf) {
			buf->reuse = true;
			list_move_tail(&buf->list, &ctx->cap_buffers);
			mutex_unlock(&ctx->mutex);
			v4l2_m2m_buf_queue(ctx->fh.m2m_ctx,
					   &buf->buffer.vb);
			break;
		}
		buf = find_buffer(res->buf_map_id, &ctx->cap_buffers);
				...
				u32 buf_map_id)
{
	...
	INIT_WORK(&res->work, return_worker);
...
static int vxd_dec_buf_prepare(struct vb2_buffer *vb)
{
	...
	if (V4L2_TYPE_IS_OUTPUT(vb->type)) {
		...
		buf->bstr_info.buf_size = q_data->size_image[plane];
				       ...
				       enum vb2_buffer_state state)
{
	...
	for (;;) {
		...
		buf = container_of(vb, struct vxd_buffer, buffer.vb);
		list_del(&buf->list);
...
static int vxd_dec_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	if (ctx->dst_streaming && ctx->src_streaming && !ctx->core_streaming) {
		if (!ctx->stream_configured) {
			vxd_dec_return_all_buffers(ctx, vq,
						   VB2_BUF_STATE_ERROR);
			return -EINVAL;
		}
...
static int vxd_dec_open(struct file *file)
{
	...
	s_q_data->bytesperline[0] = s_q_data->width;
	s_q_data->size_image[0] = s_q_data->bytesperline[0] * s_q_data->height;
	...
	ret = idr_alloc_cyclic(&vxd->streams, &ctx->stream, VXD_MIN_STREAM_ID,
...
static int vxd_dec_querycap(struct file *file, void *priv,
...
static int vxd_dec_enum_fmt(struct file *file, void *priv,
...
static struct vxd_dec_fmt *find_format(struct v4l2_format *f,
				       unsigned int type)
{
	int i;
...
static int vxd_dec_g_fmt(struct file *file, void *priv,
...
static int vxd_dec_try_fmt(struct file *file, void *priv,
			   struct v4l2_format *f)
{
	struct vxd_dec_ctx *ctx = file2ctx(file);
	struct vxd_dev *vxd_dev = ctx->dev;
				       ...
				       enum vdec_vid_std vid_std)
{
	...
	for (i = 0 ; i < MAX_SEQUENCES ; i++) {
		...
		ret = img_mem_map_km(ctx->mem_ctx,
				     fw_sequ[i].ddbuf_info.buf_id);
		if (ret) {
		...
		ret = core_stream_map_buf(ctx->res_str_id,
					  VDEC_BUFTYPE_BITSTREAM, &buf_info,
	...
	for (i = 0 ; i < MAX_PPSS ; i++) {
		...
		fw_pps[i].ddbuf_info.mem_attrib = attributes;
...
static int vxd_dec_s_fmt(struct file *file, void *priv,
			 struct v4l2_format *f)
{
	...
	if (V4L2_TYPE_IS_OUTPUT(f->type)) {
		...
		for (i = 0; i < pix_mp->num_planes; i++) {
			q_data->bytesperline[i] =
				ALIGN(q_data->width, HW_ALIGN);
			if (q_data->bytesperline[i] <
					pix_mp->plane_fmt[0].bytesperline)
				q_data->bytesperline[i] =
					ALIGN(pix_mp->plane_fmt[0].bytesperline,
					      HW_ALIGN);
...
static void device_run(void *priv)
{
	...
	if (src_vxdb->buffer.vb.flags & V4L2_BUF_FLAG_LAST)
		last = true;
	...
	item = lst_first(&preparsed_data->picture_data.pict_seg_list[0]);
	while (item) {
		next = lst_next(item);
		lst_remove(&preparsed_data->picture_data.pict_seg_list[0],
			   item);
...
static int vxd_dec_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(vxd->reg_base))
		return PTR_ERR(vxd->reg_base);
	...
	if (ret) {
		dev_err(&pdev->dev,
			"%s: main component initialisation failed!\n",
			__func__);
		goto out_init;
	}
	...
	if (i_heap_id < 0) {
		dev_err(&pdev->dev,
			"%s: Invalid internal heap id", __func__);
		goto out_init;
	}
	ret = core_initialise(vxd, i_heap_id, vxd_return_resource);
	if (ret) {
		dev_err(&pdev->dev,
			"%s: core initialization failed!", __func__);
		goto out_init;
	}
	...
	*vfd = vxd_dec_videodev;
	...
	ret = devm_request_threaded_irq(&pdev->dev, module_irq, &hard_isrcb,
					&soft_thread_irq, IRQF_SHARED,
