 ...
 * Pixel Aspect Ratio
 */
static const u16 pixel_aspect[17][2] = {
	{ 0, 1 },
	{ 1, 1 },
	{ 12, 11 },
	{ 10, 11 },
 ...
 */
static const u8 default_4x4_intra[16] = {
	6, 13, 13, 20,
	20, 20, 28, 28,
	28, 28, 32, 32,
	32, 37, 37, 42
...
static const u8 default_4x4_inter[16] = {
	10, 14, 14, 20,
	20, 20, 24, 24,
	24, 24, 27, 27,
	27, 30, 30, 34
...
static const u8 default_8x8_intra[64] = {
...
static const u8 default_8x8_inter[64] = {
 ...
 */
static const u8 default_4x4_org[16] = {
	16, 16, 16, 16,
	16, 16, 16, 16,
	16, 16, 16, 16,
	16, 16, 16, 16
 ...
 * to be use if no q matrix is chosen
 */
static const u8 default_8x8_org[64] = {
 ...
 */
static const s32 bspp_subheightc[] = { -1, 2, 1, 1 };

/*
 * source: ITU-T H.264 2010/03, page 20 Table 6-1
 */
static const s32 bspp_subwidthc[] = { -1, 2, 2, 1 };
...
static inline s32 smin(s32 a, s32 b)
{
	return (((a) < (b)) ? (a) : (b));
}

static inline s32 smax(s32 a, s32 b)
{
	return (((a) > (b)) ? (a) : (b));
}

static void set_if_not_determined_yet(s32 *determined, u8 condition, s32 *target, u32 value)
{
	do {
...
static s32 bspp_h264_get_subwidthc(s32 chroma_format_idc,
				   s32 separate_colour_plane_flag)
{
	return bspp_subwidthc[chroma_format_idc];
}

static s32 bspp_h264_get_subheightc(s32 chroma_format_idc,
				    s32 separate_colour_plane_flag)
{
	return bspp_subheightc[chroma_format_idc];
}

static u32 h264ceillog2(u32 value)
{
	u32 status = 0;
...
static enum bspp_error_type bspp_h264_scl_listparser(void *swsr_context,
						     u8 *scaling_list,
						     u8 sizeof_scaling_list,
						     u8 *usedefaultscalingmatrixflag)
{
						 ...
						 struct bspp_h264_seq_hdr_info *h264_seq_hdr_info)
{
	...
	if (sps_info->profile_idc == H264_PROFILE_HIGH ||
	    ...
	    sps_info->profile_idc == H264_PROFILE_MVC_HIGH ||
	    sps_info->profile_idc == H264_PROFILE_MVC_STEREO) {
		pr_err("This is High Profile Bitstream");
		sps_info->chroma_format_idc = swsr_read_unsigned_expgoulomb(swsr_context);
		if (sps_info->chroma_format_idc > 3) {
			pr_err("chroma_format_idc[%d] is not within the range", sps_info->chroma_format_idc);
		...
		if (sps_info->seq_scaling_matrix_present_flag) {
			pr_err("seq_scaling_matrix_present_flag is available");
			scaling_list_num = (sps_info->chroma_format_idc != 3) ? 8 : 12;

			if (!sps_info->scllst4x4seq) {
				sps_info->scllst4x4seq =
					kmalloc((sizeof(u8[H264FW_NUM_4X4_LISTS][H264FW_4X4_SIZE])),
						GFP_KERNEL);
				if (!sps_info->scllst4x4seq) {
					...
					memset(sps_info->scllst4x4seq, 0x00,
					       sizeof(u8[H264FW_NUM_4X4_LISTS][H264FW_4X4_SIZE]));
				}
			}
			if (!sps_info->scllst8x8seq) {
				sps_info->scllst8x8seq =
					kmalloc((sizeof(u8[H264FW_NUM_8X8_LISTS][H264FW_8X8_SIZE])),
						GFP_KERNEL);
				if (!sps_info->scllst8x8seq) {
					sps_parser_error |= BSPP_ERROR_OUT_OF_MEMORY;
				} else {
					...
					memset(sps_info->scllst8x8seq, 0x00,
					       sizeof(u8[H264FW_NUM_8X8_LISTS][H264FW_8X8_SIZE]));
				}
			}
			{
				u8(*scllst4x4seq)[H264FW_NUM_4X4_LISTS][H264FW_4X4_SIZE] =
					(u8 (*)[H264FW_NUM_4X4_LISTS][H264FW_4X4_SIZE])sps_info->scllst4x4seq;
				u8(*scllst8x8seq)[H264FW_NUM_8X8_LISTS][H264FW_8X8_SIZE] =
					(u8 (*)[H264FW_NUM_8X8_LISTS][H264FW_8X8_SIZE])sps_info->scllst8x8seq;

				for (i = 0; i < scaling_list_num; i++) {
					u8 *ptr = &sps_info->usedefaultscalingmatrixflag_seq[i];
					...
					if (sps_info->seq_scaling_list_present_flag[i]) {
						if (i < 6) {
							sps_parser_error |=
								bspp_h264_scl_listparser(swsr_context,
											 (*scllst4x4seq)[i],
											 16,
											 ptr);
						} else {
	...
	if (sps_info->pic_order_cnt_type > 2) {
		pr_err("pic_order_cnt_type[%d] is not within range  [0 - 2]", sps_info->pic_order_cnt_type);
		sps_parser_error |= BSPP_ERROR_INVALID_VALUE;
	}

	if (sps_info->pic_order_cnt_type == 0) {
		sps_info->log2_max_pic_order_cnt_lsb_minus4 = swsr_read_unsigned_expgoulomb(swsr_context);
		...
		sps_info->offset_for_top_to_bottom_field = swsr_read_signed_expgoulomb(swsr_context);
		sps_info->num_ref_frames_in_pic_order_cnt_cycle = swsr_read_unsigned_expgoulomb(swsr_context);
		...
		if (!sps_info->offset_for_ref_frame) {
			sps_info->offset_for_ref_frame =
				kmalloc((H264FW_MAX_CYCLE_REF_FRAMES * sizeof(u32)), GFP_KERNEL);
			if (!sps_info->offset_for_ref_frame) {
	...
	if (sps_info->max_num_ref_frames > 16) {
		pr_err("num_ref_frames[%d] is not within range [0 - 16]", sps_info->max_num_ref_frames);
		sps_parser_error |= BSPP_ERROR_INVALID_VALUE;
	}
	...
	if (sps_info->pic_width_in_mbs_minus1 >= MAX_WIDTH_IN_MBS) {
		pr_err("pic_width_in_mbs_minus1[%d] is not within range", sps_info->pic_width_in_mbs_minus1);
		sps_parser_error |= BSPP_ERROR_INVALID_VALUE;
	}
...
{
	s32 i, group, chroma_format_idc;
	u32 number_bits_per_slicegroup_id;
	u8 n_scaling_list;
	bool more_rbsp_data;
	u32 result;
	...
	if (h264_pps_info->pps_id >= MAX_PPS_COUNT) {
		...
		h264_pps_info->pps_id = BSPP_INVALID;
		return BSPP_ERROR_UNSUPPORTED;
	}
	...
	if (h264_pps_info->seq_parameter_set_id >= MAX_SPS_COUNT) {
		...
		h264_pps_info->seq_parameter_set_id = BSPP_INVALID;
		return BSPP_ERROR_UNSUPPORTED;
	}

	{
		 ...
		 * (H.7.4.1.2.1).
		 * At this point is not clear if this pps refers to an sps or a subset sps. It should be fine
		 * however for the case of chroma_format_idc to try and locate a subset sps if there isn't a
		 * normal one.
		 */
		struct bspp_h264_seq_hdr_info *h264_seq_hdr_info;
		...
		if (!seq_hdr_info) {
			seq_hdr_info =
				bspp_get_sequ_hdr(str_res, h264_pps_info->seq_parameter_set_id + 32);
			if (!seq_hdr_info)
				return BSPP_ERROR_NO_SEQUENCE_HDR;
		}

		h264_seq_hdr_info = (struct bspp_h264_seq_hdr_info *)seq_hdr_info->secure_sequence_info;
	...
	if (h264_pps_info->num_slice_groups_minus1 > 0) {
		...
		pr_err("slice_group_map_type is : %d, Parsed by BSPP", h264_pps_info->slice_group_map_type);
		...
		if (h264_pps_info->slice_group_map_type == 0) {
			...
			for (group = 0; group < h264_pps_info->num_slice_groups_minus1; group++) {
				h264_pps_info->top_left[group] = swsr_read_unsigned_expgoulomb(swsr_context);
			...
			h264_pps_info->slice_group_change_direction_flag = swsr_read_bits(swsr_context, 1);
			h264_pps_info->slice_group_change_rate_minus1 =
				swsr_read_unsigned_expgoulomb(swsr_context);
		} else if (h264_pps_info->slice_group_map_type == 6) {
			h264_pps_info->pic_size_in_map_unit = swsr_read_unsigned_expgoulomb(swsr_context);
			...
			number_bits_per_slicegroup_id = h264ceillog2(h264_pps_info->num_slice_groups_minus1 + 1);

			if ((h264_pps_info->pic_size_in_map_unit + 1) >
			    h264_pps_info->h264_ppssgm_info.slicegroupidnum) {
				u8 *slice_group_id =
					kmalloc(((h264_pps_info->pic_size_in_map_unit + 1) * sizeof(u8)),
						GFP_KERNEL);
				if (!slice_group_id) {
					pr_err("out of memory");
					...
					pr_err("reallocating SGM info from size %lu bytes to size %lu bytes",
					       h264_pps_info->h264_ppssgm_info.slicegroupidnum * sizeof(u8),
					       (h264_pps_info->pic_size_in_map_unit + 1) * sizeof(u8));
					if (h264_pps_info->h264_ppssgm_info.slice_group_id) {
						memcpy(slice_group_id,
	...
	for (i = 0; i < H264FW_MAX_REFPIC_LISTS; i++) {
		h264_pps_info->num_ref_idx_lx_active_minus1[i] = swsr_read_unsigned_expgoulomb(swsr_context);
		if (h264_pps_info->num_ref_idx_lx_active_minus1[i] >=
		    SL_MAX_REF_IDX) {
			pr_err("num_ref_idx_lx_active_minus1[%d] [%d] is not within the range", i,
			       h264_pps_info->num_ref_idx_lx_active_minus1[i]);
			pps_parse_error |= BSPP_ERROR_UNRECOVERABLE;
		}
	}
	...
	if (result == 0 && more_rbsp_data) {
		pr_err("More RBSP data is available");
		...
		if (h264_pps_info->pic_scaling_matrix_present_flag) {
			...
			{
				u8(*scllst4x4pic)[H264FW_NUM_4X4_LISTS][H264FW_4X4_SIZE] =
					(u8 (*)[H264FW_NUM_4X4_LISTS][H264FW_4X4_SIZE])h264_pps_info->scllst4x4pic;
				u8(*scllst8x8pic)[H264FW_NUM_8X8_LISTS][H264FW_8X8_SIZE] =
					(u8 (*)[H264FW_NUM_8X8_LISTS][H264FW_8X8_SIZE])h264_pps_info->scllst8x8pic;
				...
				for (i = 0; i < n_scaling_list; i++) {
					u8 *ptr = &h264_pps_info->usedefaultscalingmatrixflag_pic[i];
		...
		h264_pps_info->second_chroma_qp_index_offset = swsr_read_signed_expgoulomb(swsr_context);

		if (h264_pps_info->second_chroma_qp_index_offset > H264_MAX_CHROMA_QP_INDEX_OFFSET)
			h264_pps_info->second_chroma_qp_index_offset = H264_MAX_CHROMA_QP_INDEX_OFFSET;
		else if (h264_pps_info->second_chroma_qp_index_offset < H264_MIN_CHROMA_QP_INDEX_OFFSET)
			h264_pps_info->second_chroma_qp_index_offset = H264_MIN_CHROMA_QP_INDEX_OFFSET;
	} else {
		h264_pps_info->second_chroma_qp_index_offset = h264_pps_info->chroma_qp_index_offset;
	}
...
static s32 bspp_h264_release_sequ_hdr_info(void *str_alloc,
					   void *secure_sps_info)
{
	struct bspp_h264_seq_hdr_info *h264_seq_hdr_info = (struct bspp_h264_seq_hdr_info *)secure_sps_info;
...
static s32 bspp_h264_reset_pps_info(void *secure_pps_info)
{
	struct bspp_h264_pps_info *h264_pps_info = NULL;
	u16 slicegroupidnum = 0;
	u8 *slice_group_id = NULL;
	u8 *scllst4x4pic = NULL;
	u8 *scllst8x8pic = NULL;
	...
	if (h264_pps_info->scllst4x4pic)
		memset(h264_pps_info->scllst4x4pic, 0x00, sizeof(u8[H264FW_NUM_4X4_LISTS][H264FW_4X4_SIZE]));

	if (h264_pps_info->scllst8x8pic)
		memset(h264_pps_info->scllst8x8pic, 0x00, sizeof(u8[H264FW_NUM_8X8_LISTS][H264FW_8X8_SIZE]));
...
static enum bspp_error_type bspp_h264_pict_hdr_parser(void *swsr_context, void *str_res,
						      struct bspp_h264_slice_hdr_info *h264_slice_hdr_info,
						      struct bspp_pps_info **pps_info,
						      struct bspp_sequence_hdr_info **seq_hdr_info,
						      enum h264_nalunittype nal_unit_type, u8 nal_ref_idc)
{
	...
	h264_slice_hdr_info->slice_type = swsr_read_unsigned_expgoulomb(swsr_context);
	if ((u32)h264_slice_hdr_info->slice_type > 9) {
		pr_err("Slice Type [%d] invalid, set to P", h264_slice_hdr_info->slice_type);
		       h264_slice_hdr_info->slice_type = 0;
		slice_parse_error |= BSPP_ERROR_CORRECTION_VALIDVALUE;
	}
	h264_slice_hdr_info->slice_type = (enum bspp_h264_slice_type)(h264_slice_hdr_info->slice_type % 5);
	...
	*pps_info = pps_info_loc;
	...
	VDEC_ASSERT((u32)h264_seq_hdr_info->sps_info.seq_parameter_set_id ==
	...
	*seq_hdr_info = seq_hdr_info_loc;
	...
	if (h264_seq_hdr_info->sps_info.pic_order_cnt_type == 1 &&
		...
		h264_slice_hdr_info->delta_pic_order_cnt[0] = swsr_read_signed_expgoulomb(swsr_context);
		if (h264_pps_info->pic_order_present_flag && !h264_slice_hdr_info->field_pic_flag)
			h264_slice_hdr_info->delta_pic_order_cnt[1] = swsr_read_signed_expgoulomb(swsr_context);
	}

	if (h264_pps_info->redundant_pic_cnt_present_flag)
		h264_slice_hdr_info->redundant_pic_cnt = swsr_read_unsigned_expgoulomb(swsr_context);
	...
	if (h264_pps_info->num_slice_groups_minus1 > 0 &&
		...
		if (h264_slice_hdr_info->slice_type != SI_SLICE &&
			...
			for (list_num = 0; list_num < i; list_num++) {
				...
				if (swsr_read_bits(swsr_context, 1)) {
					do {
						command = swsr_read_unsigned_expgoulomb(swsr_context);
						if (command != 3) {
							swsr_read_unsigned_expgoulomb(swsr_context);
							cmd_num++;
						}
		...
		if ((h264_pps_info->weighted_pred_flag && h264_slice_hdr_info->slice_type == P_SLICE) ||
		    (h264_pps_info->weighted_bipred_idc && h264_slice_hdr_info->slice_type == B_SLICE)) {
			s32 mono_chrome;
			u32 list, i, j, k;
			...
			for (list = 0; list < k; list++) {
				for (i = 0; i <= h264_slice_hdr_info->num_ref_idx_lx_active_minus1[list]; i++) {
					if (swsr_read_bits(swsr_context, 1)) {
						swsr_read_signed_expgoulomb(swsr_context);
						swsr_read_signed_expgoulomb(swsr_context);
					}
		...
		if (nal_ref_idc != 0) {
			...
			if (nal_unit_type == H264_NALTYPE_IDR_SLICE) {
				...
				if (swsr_read_bits(swsr_context, 1)) {
					do {
						/* clamp 0--6 */
						memmanop = swsr_read_unsigned_expgoulomb(swsr_context);
		...
		if (h264_pps_info->entropy_coding_mode_flag && h264_slice_hdr_info->slice_type != I_SLICE)
			swsr_read_unsigned_expgoulomb(swsr_context);

		swsr_read_signed_expgoulomb(swsr_context);

		if (h264_slice_hdr_info->slice_type == SP_SLICE || h264_slice_hdr_info->slice_type == SI_SLICE) {
		...
		if (h264_pps_info->num_slice_groups_minus1 > 0 &&
		    h264_pps_info->slice_group_map_type >= 3 &&
		    h264_pps_info->slice_group_map_type <= 5) {
			u32 num_slice_group_map_units =
				(h264_seq_hdr_info->sps_info.pic_height_in_map_units_minus1 + 1) *
				(h264_seq_hdr_info->sps_info.pic_width_in_mbs_minus1 + 1);

			u16 slice_group_change_rate = (h264_pps_info->slice_group_change_rate_minus1 + 1);

			u32 width = h264ceillog2(num_slice_group_map_units / slice_group_change_rate +
				(num_slice_group_map_units % slice_group_change_rate == 0 ? 0 : 1) + 1); /* (7-32) */
			h264_slice_hdr_info->slice_group_change_cycle = swsr_read_bits(swsr_context, width);
		}
	}
					  ...
					  struct bspp_h264_seq_hdr_info *h264_seq_hdr_info)
{
	...
	u8 (*scllst4x4seq)[H264FW_NUM_4X4_LISTS][H264FW_4X4_SIZE] =
		(u8 (*)[H264FW_NUM_4X4_LISTS][H264FW_4X4_SIZE]) h264_seq_hdr_info->sps_info.scllst4x4seq;
	u8 (*scllst8x8seq)[H264FW_NUM_8X8_LISTS][H264FW_8X8_SIZE] =
		(u8 (*)[H264FW_NUM_8X8_LISTS][H264FW_8X8_SIZE]) h264_seq_hdr_info->sps_info.scllst8x8seq;
	...
	if (h264_pps_info->pic_scaling_matrix_present_flag) {
		for (i = 0; i < H264FW_NUM_4X4_LISTS; i++) {
			if (h264_pps_info->pic_scaling_list_present_flag[i]) {
				if (h264_pps_info->usedefaultscalingmatrixflag_pic[i])
					quant_matrix = (i > 2) ? default_4x4_inter : default_4x4_intra;
				else
				...
				if (h264_seq_hdr_info->sps_info.seq_scaling_matrix_present_flag) {
					...
					/* first 4x4 Inter list */
						...
						quant_matrix = h264fw_pps_info->scalinglist4x4[i - 1];
					}
				} else {
		...
		if (h264_seq_hdr_info->sps_info.seq_scaling_matrix_present_flag) {
			for (i = 0; i < H264FW_NUM_4X4_LISTS; i++) {
				if (h264_seq_hdr_info->sps_info.seq_scaling_list_present_flag[i]) {
					if (h264_seq_hdr_info->sps_info.usedefaultscalingmatrixflag_seq[i]) {
						quant_matrix = (i > 2) ? default_4x4_inter : default_4x4_intra;
					} else {
						VDEC_ASSERT(h264_seq_hdr_info->sps_info.scllst4x4seq);
						if (scllst4x4seq)
	...
	if (h264_pps_info->transform_8x8_mode_flag) {
		u8 *seq_scllstflg = h264_seq_hdr_info->sps_info.seq_scaling_list_present_flag;
		u8 *def_sclmatflg_seq = h264_seq_hdr_info->sps_info.usedefaultscalingmatrixflag_seq;

		if (h264_pps_info->pic_scaling_matrix_present_flag) {
			for (i = 0; i < num8x8_lists; i++) {
				if (h264_pps_info->pic_scaling_list_present_flag[i + H264FW_NUM_4X4_LISTS]) {
					if (h264_pps_info->usedefaultscalingmatrixflag_pic[i + H264FW_NUM_4X4_LISTS]) {
						quant_matrix = (i & 0x1) ? default_8x8_inter : default_8x8_intra;
					} else {
						VDEC_ASSERT(h264_pps_info->scllst8x8pic);
						if (scllst8x8pic)
					...
					if (h264_seq_hdr_info->sps_info.seq_scaling_matrix_present_flag) {
						...
						if (i == 0) {
							...
							quant_matrix = h264fw_pps_info->scalinglist8x8[i - 2];
						}
					} else {
						/* SPS matrix not present - use fallback rule A */
						/* list 6 - first 8x8 Intra list */
				...
				for (i = 0; i < num8x8_lists; i++) {
					if (seq_scllstflg[i + H264FW_NUM_4X4_LISTS]) {
						if (def_sclmatflg_seq[i + H264FW_NUM_4X4_LISTS]) {
							quant_matrix =
								(i & 0x1) ? default_8x8_inter : default_8x8_intra;
						} else {
							VDEC_ASSERT(h264_seq_hdr_info->sps_info.scllst8x8seq);
							if (scllst8x8seq)
								quant_matrix = (*scllst8x8seq)[i];
...
{
	h264fw_pps_info->deblocking_filter_control_present_flag = h264_pps_info->deblocking_filter_control_present_flag;
	...
	h264fw_pps_info->redundant_pic_cnt_present_flag = h264_pps_info->redundant_pic_cnt_present_flag;
	...
	VDEC_ASSERT(sizeof(h264fw_pps_info->num_ref_lx_active_minus1) == sizeof(u8) * H264FW_MAX_REFPIC_LISTS);
	memcpy(h264fw_pps_info->num_ref_lx_active_minus1, h264_pps_info->num_ref_idx_lx_active_minus1,
	...
	h264fw_pps_info->slice_group_change_rate_minus1 = h264_pps_info->slice_group_change_rate_minus1;
	h264fw_pps_info->chroma_qp_index_offset = h264_pps_info->chroma_qp_index_offset;
	h264fw_pps_info->second_chroma_qp_index_offset = h264_pps_info->second_chroma_qp_index_offset;
}
					 ...
					 struct h264fw_sequence_ps *h264_fwseq_hdr_info)
{
	...
	h264_fwseq_hdr_info->separate_colour_plane_flag = h264_seq_hdr_info->sps_info.separate_colour_plane_flag;
	h264_fwseq_hdr_info->bit_depth_luma_minus8 = h264_seq_hdr_info->sps_info.bit_depth_luma_minus8;
	h264_fwseq_hdr_info->bit_depth_chroma_minus8 = h264_seq_hdr_info->sps_info.bit_depth_chroma_minus8;
	...
	h264_fwseq_hdr_info->log2_max_frame_num = h264_seq_hdr_info->sps_info.log2_max_frame_num_minus4 + 4;
	...
	h264_fwseq_hdr_info->constraint_set_flags = h264_seq_hdr_info->sps_info.constraint_set_flags;
	...
	h264_fwseq_hdr_info->mb_adaptive_frame_field_flag = h264_seq_hdr_info->sps_info.mb_adaptive_frame_field_flag;
	h264_fwseq_hdr_info->offset_for_non_ref_pic = h264_seq_hdr_info->sps_info.offset_for_non_ref_pic;
	h264_fwseq_hdr_info->offset_for_top_to_bottom_field =
		h264_seq_hdr_info->sps_info.offset_for_top_to_bottom_field;
	h264_fwseq_hdr_info->pic_width_in_mbs_minus1 = h264_seq_hdr_info->sps_info.pic_width_in_mbs_minus1;
	h264_fwseq_hdr_info->pic_height_in_map_units_minus1 =
		h264_seq_hdr_info->sps_info.pic_height_in_map_units_minus1;
	h264_fwseq_hdr_info->direct_8x8_inference_flag = h264_seq_hdr_info->sps_info.direct_8x8_inference_flag;
	h264_fwseq_hdr_info->qpprime_y_zero_transform_bypass_flag =
		h264_seq_hdr_info->sps_info.qpprime_y_zero_transform_bypass_flag;

	if (h264_seq_hdr_info->sps_info.offset_for_ref_frame)
		memcpy(h264_fwseq_hdr_info->offset_for_ref_frame, h264_seq_hdr_info->sps_info.offset_for_ref_frame,
					     ...
					     struct vdec_comsequ_hdrinfo *comseq_hdr_info)
{
	...
	comseq_hdr_info->pixel_info.mem_pkg =
		(comseq_hdr_info->pixel_info.bitdepth_y > 8 || comseq_hdr_info->pixel_info.bitdepth_c > 8) ?
	...
	/*
	 * H264 has always coded size MB aligned. For sequences which *may* have Field-Coded pictures, as
	 * described by the frame_mbs_only_flag, the pic_height_in_map_units_minus1 refers to field height in
	 * MBs, so to find the actual Frame height we need to do Field_MBs_InHeight * 32
	 */
	...
	if (sps_info->frame_cropping_flag) {
		s32 sub_width_c, sub_height_c, crop_unit_x, crop_unit_y;
		s32 frame_crop_left, frame_crop_right, frame_crop_top, frame_crop_bottom;
		...
		comseq_hdr_info->orig_display_region.left_offset = (u32)frame_crop_left;
		comseq_hdr_info->orig_display_region.top_offset = (u32)frame_crop_top;
...
{
	/*
	 * H264 has slice coding type, not picture. The bReference contrary to the rest of the standards is set
	 * explicitly from the NAL externally (see just below the call to bspp_h264_pict_hdr_populate)
	 * pict_hdr_info->bReference = ? (Set externally for H264)
	 */
	...
	memset(pict_hdr_info->disp_info.pan_scan_windows, 0, sizeof(pict_hdr_info->disp_info.pan_scan_windows));
}

static s32 bspp_h264_destroy_seq_hdr_info(const void *secure_sps_info)
{
	struct bspp_h264_seq_hdr_info *h264_seq_hdr_info = NULL;
...
static s32 bspp_h264_destroy_pps_info(const void *secure_pps_info)
{
	struct bspp_h264_pps_info *h264_pps_info = NULL;
...
static s32 bspp_h264_destroy_data(enum bspp_unit_type data_type, void *data_handle)
{
	s32 result = 0;
...
static void bspp_h264_generate_slice_groupmap(struct bspp_h264_slice_hdr_info *h264_slice_hdr_info,
					      struct bspp_h264_seq_hdr_info *h264_seq_hdr_info,
					      struct bspp_h264_pps_info *h264_pps_info,
					      u8 *map_unit_to_slice_groupmap,
					      u32 map_size)
{
	s32 group;
	u32 num_slice_group_mapunits;
	u32 i = 0, j, k = 0;
	u8 num_slice_groups = h264_pps_info->num_slice_groups_minus1 + 1;
	u32 pic_width_in_mbs = h264_seq_hdr_info->sps_info.pic_width_in_mbs_minus1 + 1;
	u32 pic_height_in_map_units = h264_seq_hdr_info->sps_info.pic_height_in_map_units_minus1 + 1;

	num_slice_group_mapunits = map_size;
	if (h264_pps_info->slice_group_map_type == 6) {
		if ((u32)num_slice_groups != num_slice_group_mapunits) {
			VDEC_ASSERT("wrong pps->num_slice_group_map_units_minus1 for used SPS and FMO type 6");
			if (num_slice_group_mapunits > h264_pps_info->h264_ppssgm_info.slicegroupidnum)
				num_slice_group_mapunits = h264_pps_info->h264_ppssgm_info.slicegroupidnum;
		}
	}

	/* only one slice group */
	if (h264_pps_info->num_slice_groups_minus1 == 0) {
		memset(map_unit_to_slice_groupmap, 0, map_size * sizeof(u8));
		return;
	}
	...
	if (h264_pps_info->slice_group_map_type == 0) {
		do {
			for (group = 0; group <= h264_pps_info->num_slice_groups_minus1 && i < num_slice_group_mapunits;
				i += h264_pps_info->run_length_minus1[group++] + 1) {
				for (j = 0;
				     j <= h264_pps_info->run_length_minus1[group] && i + j < num_slice_group_mapunits;
				     j++)
		...
		for (i = 0; i < num_slice_group_mapunits; i++) {
			map_unit_to_slice_groupmap[i] = ((i % pic_width_in_mbs) +
				(((i / pic_width_in_mbs) * (h264_pps_info->num_slice_groups_minus1 + 1)) / 2)) %
				(h264_pps_info->num_slice_groups_minus1 + 1);
		}
	} else if (h264_pps_info->slice_group_map_type == 2) {
		u32 y_top_left, x_top_left, y_bottom_right, x_bottom_right, x, y;
		...
		for (group = h264_pps_info->num_slice_groups_minus1 - 1; group >= 0; group--) {
			...
			for (y = y_top_left; y <= y_bottom_right; y++)
				for (x = x_top_left; x <= x_bottom_right; x++) {
					if (h264_pps_info->top_left[group] > h264_pps_info->bottom_right[group] ||
					    h264_pps_info->bottom_right[group] >= num_slice_group_mapunits)
						continue;
					map_unit_to_slice_groupmap[y * pic_width_in_mbs + x] = group;
				}
		}
	} else if (h264_pps_info->slice_group_map_type == 3) {
		s32 left_bound, top_bound, right_bound, bottom_bound;
		s32 x, y, x_dir, y_dir;
		s32 map_unit_vacant;

		u32 mapunits_in_slicegroup_0 =
			umin((u32)((h264_pps_info->slice_group_change_rate_minus1 + 1) *
		...
		y = (pic_height_in_map_units - h264_pps_info->slice_group_change_direction_flag) / 2;
		...
		for (k = 0; k < num_slice_group_mapunits; k += map_unit_vacant) {
			...
			if (x_dir == -1 && x == left_bound) {
				...
				right_bound = smin(right_bound + 1,
						   (s32)pic_width_in_mbs - 1);
				x = right_bound;
				x_dir = 0;
				...
				bottom_bound = smin(bottom_bound + 1, (s32)pic_height_in_map_units - 1);
				y = bottom_bound;
		...
		u32 mapunits_in_slicegroup_0 =
			umin((u32)((h264_pps_info->slice_group_change_rate_minus1 + 1) *
				h264_slice_hdr_info->slice_group_change_cycle),
			     (u32)num_slice_group_mapunits);
		u32 sizeof_upper_left_group =
			h264_pps_info->slice_group_change_direction_flag ?
				(num_slice_group_mapunits - mapunits_in_slicegroup_0) : mapunits_in_slicegroup_0;
		for (i = 0; i < num_slice_group_mapunits; i++) {
			if (i < sizeof_upper_left_group)
				map_unit_to_slice_groupmap[i] = h264_pps_info->slice_group_change_direction_flag;

			else
				map_unit_to_slice_groupmap[i] = 1 - h264_pps_info->slice_group_change_direction_flag;
		}
		...
		for (j = 0; j < (u32)pic_width_in_mbs; j++) {
			for (i = 0; i < (u32)pic_height_in_map_units; i++) {
				if (k++ < sizeof_upper_left_group)
					map_unit_to_slice_groupmap[i * pic_width_in_mbs + j] =
						h264_pps_info->slice_group_change_direction_flag;
				else
					map_unit_to_slice_groupmap[i * pic_width_in_mbs + j] =
						1 - h264_pps_info->slice_group_change_direction_flag;
			}
		}
	} else if (h264_pps_info->slice_group_map_type == 6) {
		VDEC_ASSERT(num_slice_group_mapunits <= h264_pps_info->h264_ppssgm_info.slicegroupidnum);
		for (i = 0; i < num_slice_group_mapunits; i++)
			map_unit_to_slice_groupmap[i] = h264_pps_info->h264_ppssgm_info.slice_group_id[i];
	}
}

static s32 bspp_h264_parse_mvc_slice_extension(void *swsr_context, struct bspp_h264_inter_pict_ctx *inter_pict_ctx)
{
	if (!swsr_read_bits(swsr_context, 1)) {
...
static s32 bspp_h264_unitparser_compile_sgmdata(struct bspp_h264_slice_hdr_info *h264_slice_hdr_info,
						struct bspp_h264_seq_hdr_info *h264_seq_hdr_info,
						struct bspp_h264_pps_info *h264_pps_info,
						struct bspp_pict_hdr_info *pict_hdr_info)
{
	...
	pict_hdr_info->pict_sgm_data.size = (h264_seq_hdr_info->sps_info.pic_height_in_map_units_minus1 + 1) *
		(h264_seq_hdr_info->sps_info.pic_width_in_mbs_minus1 + 1);

	pict_hdr_info->pict_sgm_data.pic_data = kmalloc((pict_hdr_info->pict_sgm_data.size), GFP_KERNEL);
	...
	bspp_h264_generate_slice_groupmap(h264_slice_hdr_info, h264_seq_hdr_info, h264_pps_info,
					  pict_hdr_info->pict_sgm_data.pic_data, pict_hdr_info->pict_sgm_data.size);
...
static s32 bspp_h264_unit_parser(void *swsr_context, struct bspp_unit_data *unit_data)
{
	u32 result = 0;
	enum bspp_error_type parse_error = BSPP_ERROR_NONE;
	...
	case BSPP_UNIT_SEQUENCE:
			    ...
			    nal_unit_type == H264_NALTYPE_SUBSET_SPS);
		{
			...
			struct h264fw_sequence_ps *h264_fwseq_hdr_info =
				(struct h264fw_sequence_ps *)((u8 *)tmp->ddbuf_info.cpu_virt_addr + tmp->buf_offset);
			/* Common Sequence Header Info */
			struct vdec_comsequ_hdrinfo *comseq_hdr_info = &out_seq_info->sequ_hdr_info.com_sequ_hdr_info;

			pr_err("Unit Parser:Found SEQUENCE_PARAMETER_SET NAL unit");
			...
			/* Set the SPS ID */
			/*
			 * seq_parameter_set_id is always in range 0-31, so we can add offset indicating subsequence
			 * header
			 */
	...
	case BSPP_UNIT_PPS:
		VDEC_ASSERT(nal_unit_type == H264_NALTYPE_PICTURE_PARAMETER_SET);
		{
			...
			struct h264fw_picture_ps *h264fw_pps_info =
				(struct h264fw_picture_ps *)((u8 *)tmp->ddbuf_info.cpu_virt_addr + tmp->buf_offset);

			pr_err("Unit Parser:Found PICTURE_PARAMETER_SET NAL unit");
	...
	case BSPP_UNIT_PICTURE:
		if (nal_unit_type == H264_NALTYPE_SLICE_PREFIX) {
			...
			s32 current_pic_is_new = 0;
			s32 determined = 0;
			s32 id_loc;

			pr_err("Unit Parser:Found PICTURE DATA unit");
			...
			do {
				 ...
				 * works.
				 */
				unit_data->new_closed_gop = h264_slice_hdr_info.slice_type == I_SLICE ? 1 : 0;
				...
				h264fw_pps_info =
					(struct h264fw_picture_ps *)((u8 *)tmp1->ddbuf_info.cpu_virt_addr +
								     tmp1->buf_offset);
				h264_fwseq_hdr_info =
					(struct h264fw_sequence_ps *)((u8 *)tmp2->ddbuf_info.cpu_virt_addr +
								      tmp2->buf_offset);
				VDEC_ASSERT(h264_slice_hdr_info.pps_id == h264_pps_info->pps_id);
				VDEC_ASSERT(h264_pps_info->seq_parameter_set_id ==
					    (u32)h264_seq_hdr_info->sps_info.seq_parameter_set_id);

				/*
				 * Update the decoding-related FW SPS info related to the current picture with the SEI
				 * data that were potentially received and also relate to the current info. Until we
				 * receive the picture we do not know which sequence to update with the SEI data. Set
				 * from last SEI, needed for decoding
				 */
				...
				/*
				 * Determine if current slice is a new picture, and update the related params for future
				 * reference
				 ...
				 */
				{
					...
					set_if_not_determined_yet(&determined, state->new_view, &current_pic_is_new, 1);
					...
					set_if_not_determined_yet(&determined,
								  ...
								  1);
					set_if_not_determined_yet(&determined,
								  ((h264_seq_hdr_info->sps_info.pic_order_cnt_type ==
								    0) &&
					...
					set_if_not_determined_yet(&determined,
								  ((state->prev_nal_unit_type ==
								    (s32)H264_NALTYPE_IDR_SLICE ||
								    nal_unit_type == (s32)H264_NALTYPE_IDR_SLICE) &&
								   (state->prev_nal_unit_type !=
								    (s32)nal_unit_type)),
								  &current_pic_is_new,
								  1);
					...
					/*
					 * Update whatever is not updated already in different places of the code or
					 * just needs to be updated here
					 */
					...
					state->prev_delta_pic_order_cnt[1] =
						h264_slice_hdr_info.delta_pic_order_cnt[1];
					state->prev_nal_unit_type = (s32)nal_unit_type;
				...
				if (h264_slice_hdr_info.field_pic_flag && current_pic_is_new) {
					...
					if (unit_data->parse_state->prev_bottom_pic_flag !=
						(u8)BSPP_INVALID) {
						/* We should have not flagged 2nd field already */
						VDEC_ASSERT(unit_data->parse_state->second_field_flag == 0);
						/* If 2 fields in same buffer, they should be a pair */
						VDEC_ASSERT(unit_data->parse_state->prev_bottom_pic_flag !=
							    h264_slice_hdr_info.bottom_field_flag);
						unit_data->parse_state->second_field_flag = 1;
					}
				 ...
				 * We want to populate the VDEC Picture Header Info only once per picture/field, not
				 * every slice
				 */
				if (current_pic_is_new) {
					...
					if (h264_pps_info->num_slice_groups_minus1 > 0 &&
						...
						bspp_h264_unitparser_compile_sgmdata(&h264_slice_hdr_info,
										     h264_seq_hdr_info,
										     h264_pps_info,
						...
						unit_data->out.pict_hdr_info->pict_sgm_data.id = BSPP_INVALID;
					...
					/*
					 * Select the scaling lists based on h264_pps_info and h264_seq_hdr_info and
					 * pass them to h264fw_pps_info
					 */
					...
					unit_data->out.pict_hdr_info->h264_pict_hdr_info.frame_num =
						h264_slice_hdr_info.frame_num;
					unit_data->out.pict_hdr_info->h264_pict_hdr_info.nal_ref_idc = nal_ref_idc;

					/*
					 * Update the display-related picture header information with the related
					 * SEI parsed data
					 * The display-related SEI is used only for the first picture after the SEI
					 */
					if (!interpicctx->sei_info_attached_to_pic) {
						...
						if (interpicctx->active_sps_for_sei_parsing !=
						    h264_seq_hdr_info->sps_info.seq_parameter_set_id) {
							/*
							 * We tried to guess the SPS ID that we should use to parse the
							 * SEI, but we guessed wrong
							 */
					...
					if (unit_data->parse_state->second_field_flag)
						...
						type_pict_aux_data = &unit_data->out.pict_hdr_info->pict_aux_data;
				...
				/* Return the SPS ID */
				/*
				 * seq_parameter_set_id is always in range 0-31, so we can add offset indicating
				 * subsequence header
				 */
				id_loc = h264_pps_info->seq_parameter_set_id;
...
static s32 bspp_h264releasedata(void *str_alloc, enum bspp_unit_type data_type, void *data_handle)
{
	s32 result = 0;
	...
	switch (data_type) {
	case BSPP_UNIT_SEQUENCE:
		result = bspp_h264_release_sequ_hdr_info(str_alloc,
							 data_handle);
		break;
	default:
		break;
...
static s32 bspp_h264resetdata(enum bspp_unit_type data_type, void *data_handle)
{
	s32 result = 0;
...
static void bspp_h264update_unitcounts(void *swsr_ctx, u32 *unitcount, u32 *unit_arraycount)
{
	if (*unitcount == 0) {
		u64 value = 1;
	...
	struct bspp_inter_pict_data *pinterpict_data)
{
	...
	if (bstr_format == VDEC_BSTRFORMAT_DEMUX_BYTESTREAM || bstr_format == VDEC_BSTRFORMAT_ELEMENTARY) {
...
{
	u8 type = bitstream_unittype & 0x1f;
