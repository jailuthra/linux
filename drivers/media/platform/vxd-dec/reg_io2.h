...
#define REGIO_READ_REPEATED_FIELD(regval, group, reg, field, repidx) ({ \
	 ...
	 */
	#define REGIO_CHECK_VALUE_FITS_WITHIN_FIELD(group, reg, field, value) \
	{								\
	typeof(value) __value = value; \
	u32 temp = (u32)(__value);					\
		if (temp > group##_##reg##_##field##_LSBMASK) {	\
			IMG_ASSERT((((u32)__value) & \
			(u32)~(group##_##reg##_##field##_LSBMASK)) ==	\
			(u32)~(group##_##reg##_##field##_LSBMASK));	\
		}							\
	}
#else
...
#define REGIO_WRITE_FIELD(regval, group, reg, field, value)	\
{								\
	typeof(regval) __regval = regval; \
	typeof(value) __value = value; \
	REGIO_CHECK_VALUE_FITS_WITHIN_FIELD(group, reg, field, __value); \
	(regval) =							\
	((__regval) & ~(group##_##reg##_##field##_MASK)) |		\
		(((u32)(__value) << (group##_##reg##_##field##_SHIFT)) & \
				(group##_##reg##_##field##_MASK));	\
}

/* This macro is used to update the value of a field in a register. */
#define REGIO_WRITE_FIELD_LITE(regval, group, reg, field, value)	\
{									\
	typeof(value) __value = value; \
	REGIO_CHECK_VALUE_FITS_WITHIN_FIELD(group, reg, field, __value); \
	(regval) |= ((u32)(__value) << (group##_##reg##_##field##_SHIFT)); \
}
...
#define REGIO_ENCODE_FIELD(group, reg, field, value)    \
		((u32)(value) << (group##_##reg##_##field##_SHIFT))
