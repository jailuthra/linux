...
#define NBIT_8BYTE_MASK(n)    (((u64)1 << (n)) - 1)
...
static u64 left_aligned_nbit_8byte_mask(u32 mask, u32 nbits)
{
	return (((u64)mask << (64 - nbits)) | (u64)NBIT_8BYTE_MASK(64 - nbits));
}

/*
 * buffer has been exhausted and there is still more bytes declared in bitstream
 */
static int swsr_extractbyte(struct swsr_context *ctx, u8 *byte_ext)
{
	struct swsr_input *input;
...
static bool swsr_checkfor_delimiter(struct swsr_context *ctx)
{
	struct swsr_input *input;
	bool delim_found = 0;
...
static enum swsr_found swsr_readbyte_from_inputfifo
	 ...
	 u8 *byte)
{
	...
	if (input->num_bytes == 0) {
		...
		if (input->emprevent != SWSR_EMPREVENT_NONE &&
			...
			bool emprev_removed = 0;
			u32 shift = (SWSR_INPUT_FIFO_LENGTH - input->emprev_seq_len) * 8;
			u64 sequ = input->fifo >> shift;
			...
			if (input->emprevent == SWSR_EMPREVENT_00000300) {
				if ((sequ & 0xffffff00) == 0x00000300) {
					...
					/*
					 * Instead of trying to remove the emulation prevention byte from
					 * the middle of the FIFO simply make it zero and drop the next byte from the
					 * FIFO which will also be zero.
					 */
					input->fifo &=
						left_aligned_nbit_8byte_mask(0xffff00ff,
				...
				if (sequ == 0xff00) {
					...
					input->fifo <<= 8;
					input->fifo |= ((u64)0xff00 << shift);
					emprev_removed = 1;
				}
			} else if (input->emprevent == SWSR_EMPREVENT_000002) {
				/*
				 * Remove the emulation prevention bytes if we find 22 consecutive 0 bits
				 * (from a byte-aligned position?!)
				 */
				if (sequ == 0x000002) {
					/*
					 * Appear to "remove" the 0x02 byte by clearing it and then dropping
					 * the top (zero) byte.
					 */
					input->fifo &=
						left_aligned_nbit_8byte_mask(0xffff00,
									     input->emprev_seq_len * 8);
					input->fifo <<= 8;
					emprev_removed = 1;
				}
		...
		*byte = (u8)(input->fifo >> ((SWSR_INPUT_FIFO_LENGTH - 1) * 8));
		input->fifo <<= 8;
...
(struct swsr_context *ctx, u8 *byte)
{
...
static u32 swsr_getbits_from_outputfifo(struct swsr_context *ctx,
					u32 numbits, bool bconsume)
{
	u32 bitsread;
	u32 result;
	...
	if (numbits > ctx->output.num_bits) {
		...
		bitsread = (u32)(ctx->output.fifo >> shift);
...
static u32 swsr_readunsigned_expgoulomb(struct swsr_context *ctx)
{
	u32 numbits = 0;
	u32 bitpeeked;
	u32 bitread;
...
u32 swsr_read_unsigned_expgoulomb(void *ctx_hndl)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
	u32 value;
...
int swsr_check_more_rbsp_data(void *ctx_hndl, bool *more_rbsp_data)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
	...
	if (ctx->output.num_bits != 0) {
		...
		rest_alignedbytes = (ctx->output.fifo >>
			...
			(((u64)1 << numof_aligned_rembits) - 1);

		if ((currentbyte == (1 << (rembitsinbyte - 1))) &&
		    (numof_aligned_rembits == 0 || (rest_alignedbytes == 0 &&
		     ((((((u32)numof_aligned_rembits >> 3)) < ctx->input.emprev_seq_len) &&
...
u32 swsr_read_onebit(void *ctx_hndl)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
	u32 bitread;
...
u32 swsr_read_bits(void *ctx_hndl, u32 no_bits)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
...
int swsr_read_signedbits(void *ctx_hndl, u32 no_bits)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
	int outbits = 0;
...
u32 swsr_peekbits(void *ctx_hndl, u32 no_bits)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
...
int swsr_get_total_bitsconsumed(void *ctx_hndl, u64 *total_bitsconsumed)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
	...
	if (total_bitsconsumed)
		*total_bitsconsumed = ctx->output.totalbits_consumed;

	return 0;
}

int swsr_get_byte_offset_curbuf(void *ctx_hndl, u64 *byte_offset)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
...
int swsr_consume_delim(void *ctx_hndl, enum swsr_emprevent emprevent,
		       u32 size_delim_length, u64 *byte_count)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
	struct swsr_input *input;
	u64 delimiter = 0;
	...
	if (input->config.delim_length) {
		 ...
		 */
		shift = SWSR_OUTPUT_FIFO_LENGTH - (input->config.delim_length + SWSR_NALTYPE_LENGTH);
		 ...
		 */
		ctx->buffer_ctx.cb_fxn(SWSR_EVENT_DELIMITER_NAL_TYPE,
				       ctx->buffer_ctx.cb_param,
				       naltype,
				       NULL,
				       NULL);
	}
	...
	if (input->config.delim_length != 0) {
		u64 scpvalue = input->config.scp_value;
		u32 i;
		u8 byte = 0;
		...
		while (found == SWSR_FOUND_DATA) {
			...
			while (found == SWSR_FOUND_DATA) {
				offset = memchr(input->buf->data + input->buf->byte_offset,
						scpfirstbyte,
						input->buf->num_bytes -
						(input->buf->byte_offset + delimlength_inbytes - 1));
				...
				if (offset) {
					...
					if ((input->fifo & mask) == scp) {
						...
						input->buf->num_bytes_read = offset - input->buf->data;
					...
					u32 bytesread = input->buf->num_bytes -
					...
					for (i = 0;
					     ...
					     i++) {
						found =
						swsr_consumebyte_from_inputfifo(ctx,
										&byte);
...
int swsr_start_bitstream(void *ctx_hndl, const struct swsr_config *config,
			 u64 bitstream_size, enum swsr_emprevent emprevent)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
	struct swsr_buffer *buffer;
	u32 result;
	...
	void *cb_param, void **ctx_hndl)
{
	...
	SWSR_ASSERT(SWSR_MAX_SYNTAX_LENGTH <= (sizeof(u32) * 8));
...
static bool swsr_israwdata_extraction_supported(struct swsr_context *ctx)
{
	/*
...
static int swsr_getcurrent_delimited_unitsize(struct swsr_context *ctx,
					      u32 *size)
{
	struct swsr_buffer *buf;
	...
	if (buf && ctx->input.delimited_unit_start_offset >= 0 &&
		...
		u64 bufptr = (u64)ctx->input.delimited_unit_start_offset;
		u32 zeros = 0;
		...
		ctx->input.delimited_unit_size =
			(u32)(bufptr -
			(u64)ctx->input.delimited_unit_start_offset) - zeros;
...
int swsr_get_current_delimited_unitsize(void *ctx_hndl, u32 *size)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
...
int swsr_get_current_delimited_unit(void *ctx_hndl, u8 *data, u32 *size)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
	struct swsr_buffer *buf;
	u32 copysize;
...
int swsr_get_current_delimited_unit_bit_offset(void *ctx_hndl, u32 *bit_offset)
{
	struct swsr_context *ctx = (struct swsr_context *)ctx_hndl;
