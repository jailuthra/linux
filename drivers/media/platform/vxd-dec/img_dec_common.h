...
#define PVDEC_HEAP_BITSTREAM_START  0xE2000000ul
...
struct vxd_core_props {
	...
	u32 mmu_config1;
	u32 mtx_ram_size;
	u32 pixel_max_frame_cfg;
	u32 pixel_pipe_cfg[VXD_MAX_PIPES];
	u32 pixel_misc_cfg[VXD_MAX_PIPES];
	u32 dbg_fifo_size;
};

struct vxd_alloc_data {
	u32 heap_id;       /* [IN] Heap ID of allocator                */
	u32 size;          /* [IN] Size of device memory (in bytes)    */
	u32 attributes;    /* [IN] Attributes of buffer */
	u32 buf_id;        /* [OUT] Generated buffer ID                */
};

struct vxd_free_data {
	u32 buf_id;        /* [IN] ID of device buffer to free */
};
