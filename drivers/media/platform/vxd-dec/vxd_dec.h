 ...
 */
typedef void (*vxd_cb)(void *ctx, enum vxd_cb_type type, u32 buf_map_id);
...
struct vxd_dev {
	...
	u32 hw_on;
	u32 hw_dead;
	spinlock_t lock; /* basic device level spinlock */
...
typedef void (*decode_cb)(int res_str_id, u32 *msg, u32 msg_size,
			  u32 msg_flags);
...
struct vxd_dec_ctx {
	struct v4l2_fh fh;
	struct vxd_dev *dev;
	struct v4l2_ctrl_handler ctrl_hdl_dec;
	struct mem_ctx *mem_ctx;
	struct mmu_ctx *mmu_ctx;
	unsigned int ptd;
...
int vxd_map_buffer_sg(struct vxd_dev *vxd, struct vxd_dec_ctx *ctx,
		      u32 str_id, u32 buff_id, struct sg_table *sgt,
		      u32 virt_addr, u32 map_flags);
