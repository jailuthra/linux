...
/* Global context. */
static struct addr_context global_ctx;
...
{
	u32 result = IMG_ERROR_ALREADY_INITIALISED;
...
int addr_cx_initialise(struct addr_context * const context)
{
	...
	if (!global_initialized) {
		...
		if (!global_lock) {
			/* Create one */
			global_lock = kzalloc(sizeof(*global_lock),
					      GFP_KERNEL);
			if (!global_lock)
...
int addr_malloc(const char * const name,
		u64 size,
		u64 * const base_adr)
{
...
int addr_free(const char * const name, u64 addr)
{
