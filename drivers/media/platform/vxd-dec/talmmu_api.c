...
static struct lst_t gl_dmtmpl_lst;
				  ...
				  void **devmem_template_hndl)
{
	...
	devmem_template->pagetable_entries_perpage =
		(devmem_template->devmem_info.page_size / sizeof(u32));
			      ...
			      struct talmmu_devmem_heap **devmem_heap_arg)
{
	...
	if (isfull) {
		...
		pgnum_shift = devmem_template->page_num_shift;
		heap_info_sz = devmem_heap->heap_info.size;
		heap_num_pgs = devmem_heap->num_pages;
		ptentries_ppg = devmem_template->pagetable_entries_perpage;
		ptnum_shift = devmem_template->pagetable_num_shift;

		/* Update the number of pages */
		devmem_heap->num_pages = (heap_info_sz >> pgnum_shift);

		/* Update the number of pagetables */
		devmem_heap->num_pagetables =
			(heap_num_pgs + ptentries_ppg - 1) >> ptnum_shift;

		num_pgts = devmem_heap->num_pagetables;

		/* Allocate memory for the pagetables */
		devmem_heap->pagetable =
			kzalloc((sizeof(*devmem_heap->pagetable) * num_pgts),
				GFP_KERNEL);

		if (!devmem_heap->pagetable) {
			kfree(devmem_heap);
			return IMG_ERROR_OUT_OF_MEMORY;
		}
	}
				      ...
				      void **mem_hndl)
{
	...
	mem->dev_virtoffset = (u32)ui64_dev_offset;
	if (res != IMG_SUCCESS)
		/*
