...
#define HAS_X_REACHED_Y(x, y, range) ({ \
		typeof(x) __x = x; \
		typeof(y) __y = y; \
		typeof(range) __range = range; \
...
#define HAS_X_PASSED_Y(x, y, range) ({ \
	typeof(x) __x = x; \
	typeof(y) __y = y; \
	typeof(range) __range = range; \
	(((((__x) - (__y) + (__range)) % (__range)) < \
...
struct vdecfw_seqpps_locations {
	IMG_ALIGN_FIELD(VDECFW_SHARE_PTR_ALIGNMENT,
			uintptr_t, sequence_info_dest);
	IMG_ALIGN_FIELD(VDECFW_SHARE_PTR_ALIGNMENT,
			uintptr_t, vps_info_dest);
	IMG_ALIGN_FIELD(VDECFW_SHARE_PTR_ALIGNMENT,
			uintptr_t, pps_info_dest);
	IMG_ALIGN_FIELD(VDECFW_SHARE_PTR_ALIGNMENT,
			uintptr_t, second_pps_info_dest);
