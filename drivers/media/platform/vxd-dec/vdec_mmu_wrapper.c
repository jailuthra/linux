...
static u32 set_attributes(enum sys_emem_attrib mem_attrib)
{
	u32 attrib = 0;
 ...
 */
static int mmu_devmem_context_create(struct mmu_dev_context *dev_ctx,
				     void **mmu_ctx_hndl)
{
	int result;
	...
	for (i = 0; i < MMU_HEAP_MAX; i++) {
		...
		guardband_option = heap_opt1.guardband_opt.guardband;
		heap_option_id = TALMMU_HEAP_OPT_ADD_GUARD_BAND;
		if (use_guardband && guardband_option > 0) {
			result = talmmu_devmem_heap_options(devmem_heap_hndl,
							    heap_option_id,
							    heap_opt1);
			if (result != IMG_SUCCESS)
				return result;
		     ...
		     struct vxdio_ddbufinfo *ddbuf_info)
{
	int result;
	void *devmem_heap_hndl;
	u32 dev_page_size = DEV_MMU_PAGE_SIZE;
 ...
 */
int mmu_free_mem(void *mmustr_hndl,
		 struct vxdio_ddbufinfo *ddbuf_info)
{
	int tmp_result;
	int result = IMG_SUCCESS;
 ...
 * @Function	mmu_free_mem
 */
int mmu_free_mem_sg(void *mmustr_hndl,
		    struct vxdio_ddbufinfo *ddbuf_info)
{
	int tmp_result;
	int result = IMG_SUCCESS;
 ...
 */
int mmu_get_heap(u32 image_stride, enum mmu_eheap_id *heap_id)
{
	u32 i;
	u8 found = false;
