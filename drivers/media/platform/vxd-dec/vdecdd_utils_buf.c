 ...
 */
static inline
u8 is_packedbuf_chroma_aligned(u32 offset, u32 color_plane, u32 align)
{
 ...
 */
static u32 h264_max_dpb_mbs[H264_LEVEL_MAJOR_NUM][H264_LEVEL_MINOR_NUM] = {
...
typedef int (*fn_ref_pic_get_max_num)(const struct vdec_comsequ_hdrinfo
					*comseq_hdrinfo, u32 *max_ref_pic_num);
...
vdecddutils_ref_pict_get_maxnum(const struct vdec_str_configdata *str_cfg_data,
				const struct vdec_comsequ_hdrinfo
						*comseq_hdr_info,
				u32 *num_picts)
{
	int ret = IMG_SUCCESS;
	...
	if (str_cfg_data->vid_std == VDEC_STD_UNDEFINED ||
	    str_cfg_data->vid_std >= VDEC_STD_MAX)
		return IMG_ERROR_VALUE_OUT_OF_RANGE;
...
static int
vdecddutils_update_rend_pictsize(struct vdec_pict_size pict_size,
...
int
vdecddutils_convert_buffer_config(const struct vdec_str_configdata
					*str_cfg_data,
				  const struct vdec_pict_bufconfig
					*pict_bufcfg,
				 struct vdec_pict_rendinfo *pict_rend_info)
{
	...
	u32	ret;
	u32	i;
	u32	total_vert_samples = 0;
	u32	vert_samples[IMG_MAX_NUM_PLANES];
	u32	plane_size = 0;
	u32	plane_offset = 0;
	...
	for (i = 0; i < IMG_MAX_NUM_PLANES; i++) {
		if (pixfmt.planes[i]) {
			u32 plane_align = VDEC_VXD_PICTBUF_ALIGNMENT;
			...
			if (!pict_bufcfg->packed &&
				...
				if (pict_bufcfg->chroma_offset[i] &&
			...
			pict_size.height =
				(vert_samples[i] * pixfmt.v_denom) /
				pixfmt.v_numer[i];
			ret = vdecddutils_update_rend_pictsize(pict_size,
...
static u8 vdecddutils_is_secondary_op_required(const struct vdec_comsequ_hdrinfo
					       *comseq_hdr_info,
...
int
vdecddutils_get_minrequired_numpicts(const struct vdec_str_configdata
					*str_cfg_data,
				     const struct vdec_comsequ_hdrinfo
					*comseq_hdr_info,
				     ...
				     u32 *num_picts)
{
	...
	if (vdecddutils_is_secondary_op_required(comseq_hdr_info, op_cfg)) {
		 ...
		 */
		ret = vdecddutils_ref_pict_get_maxnum(str_cfg_data,
						      comseq_hdr_info,
						      &max_held_picnum);
		if (ret != IMG_SUCCESS)
			return ret;
...
static int
vdecddutils_get_codedsize(const struct vdec_pict_rend_config *pict_rend_cfg,
...
static u8
vdecddutils_is_packed(const struct vdec_pict_rendinfo *pict_rend_info,
		      const struct vdec_pict_rend_config *pict_rend_cfg)
{
	...
	if (pict_rend_info->plane_info[VDEC_PLANE_VIDEO_Y].size !=
		...
		packed = false;
	} else if (!is_packedbuf_chroma_aligned(pict_rend_info->plane_info[VDEC_PLANE_VIDEO_UV].offset,
						VDEC_PLANE_VIDEO_Y,
						pict_buf_align)) {
...
static int vdecddutils_get_stride
	(const struct vdec_str_configdata *str_cfg_data,
	 const struct vdec_pict_rend_config *pict_rend_cfg,
	 u32 vert_samples, u32 *h_stride,
	 enum vdec_color_planes color_planes)
{
	u32 hw_h_stride = *h_stride;
...
static int
vdecddutils_get_render_info(const struct vdec_str_configdata *str_cfg_data,
			    const struct vdec_pict_rend_config *pict_rend_cfg,
			    const struct pixel_pixinfo *pix_info,
			    struct vdec_pict_rendinfo *pict_rend_info)
{
	...
	u8	single_stride = false;
	u32	vert_sample[IMG_MAX_NUM_PLANES];
	u32	total_vert_samples;
	u32	largest_stride;
	u32	result;
	...
	result = vdecddutils_get_codedsize(pict_rend_cfg, &coded_pict_size);
	...
	for (i = 0; i < IMG_MAX_NUM_PLANES; i++) {
		if (pixfmt.planes[i]) {
			 ...
			 * (inc. constraints).
			 */
			result = vdecddutils_get_stride(str_cfg_data,
							pict_rend_cfg,
							vert_sample[i],
							&h_stride, i);
			VDEC_ASSERT(result == IMG_SUCCESS ||
			...
			if (result != IMG_SUCCESS) {
				if (result == IMG_ERROR_NOT_SUPPORTED)
					pr_err("No valid VXD stride found for picture with decoded dimensions [%d x %d] and min stride [%d]",
					       coded_pict_size.width,
					       coded_pict_size.height,
					       h_stride);
				return result;
			}
	...
	for (i = 0; i < IMG_MAX_NUM_PLANES; i++) {
		if (pixfmt.planes[i]) {
			struct vdec_pict_size  pict_size;
			u32 vert_samples = vert_sample[i];
			u32 plane_align = VDEC_VXD_PICTBUF_ALIGNMENT;
			...
			if (i == 1 && pict_rend_cfg->packed)
				...
				VDEC_ASSERT(pict_rend_info->rendered_size ==
					    ALIGN(pict_rend_info->rendered_size,
						  plane_align));
			...
			pict_size.height = (vert_sample[i] * pixfmt.v_denom) /
						pixfmt.v_numer[i];

			result = vdecddutils_update_rend_pictsize(pict_size,
								  &pict_rend_info->rend_pict_size);
			VDEC_ASSERT(result == IMG_SUCCESS);
			if (result != IMG_SUCCESS)
				return result;
...
int
vdecddutils_pictbuf_getconfig(const struct vdec_str_configdata *str_cfg_data,
			      const struct vdec_pict_rend_config *pict_rend_cfg,
			      ...
			      struct vdec_pict_bufconfig *pict_bufcfg)
{
	...
	/* Get the coded size for the appropriate orientation */
	ret = vdecddutils_get_codedsize(pict_rend_cfg, &coded_pict_size);
	VDEC_ASSERT(ret == IMG_SUCCESS);
	if (ret != IMG_SUCCESS)
		return ret;
	...
	for (i = 0; i < VDEC_PLANE_MAX; i++)
		pict_bufcfg->stride[i] =
			disp_pict_rendinfo.plane_info[i].stride;
	...
	pict_bufcfg->chroma_offset[0] =
		disp_pict_rendinfo.plane_info[VDEC_PLANE_VIDEO_UV].offset;
	pict_bufcfg->chroma_offset[1] =
		disp_pict_rendinfo.plane_info[VDEC_PLANE_VIDEO_V].offset;

	if (pict_rend_cfg->packed && str_opcfg->pixel_info.num_planes > 1) {
		pict_bufcfg->packed =
			vdecddutils_is_packed(&disp_pict_rendinfo,
					      pict_rend_cfg);
		if (!pict_bufcfg->packed) {
		...
		if (pict_rend_cfg->packed) {
			u32 size0 = ALIGN(pict_bufcfg->chroma_offset[0],
					  VDEC_VXD_PICTBUF_ALIGNMENT);
			u32 size1 = ALIGN(pict_bufcfg->chroma_offset[1],
					  VDEC_VXD_PICTBUF_ALIGNMENT);
...
int
vdecddutils_get_display_region(const struct vdec_pict_size *coded_size,
			       ...
			       struct vdec_rect *disp_region)
{
	...
	*disp_region = *orig_disp_region;
	...
	if (orig_disp_region->height == 0 || orig_disp_region->width == 0 ||
		...
		pr_err("Display Size: [%d,%d]",
		       orig_disp_region->width, orig_disp_region->height);
		pr_err("Coded Size  : [%d,%d]",
...
int
vdecddutils_pictbuf_getinfo(const struct vdec_str_configdata *str_cfg_data,
			    const struct vdec_pict_rend_config *pict_rend_cfg,
