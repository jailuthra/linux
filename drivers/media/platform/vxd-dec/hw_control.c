...
static struct vdeckm_context acore_ctx[VXD_MAX_CORES];
...
static int vdeckm_handle_mtxtohost_msg(u32 *msg, struct lst_t *pend_pict_list,
				       enum vxd_msg_attr *msg_attr,
				       struct dec_decpict  **decpict,
				       u8 msg_type, u32 trans_id)
{
	...
	if (flag) {
		...
		pict_attrs->fe_err = (u32)error_flags;
	...
	*decpict = pdec_pict;
	...
	case FW_DEVA_PANIC:
	{
	u32  panic_info =  MEMIO_READ_FIELD(msg, FW_DEVA_PANIC_ERROR_INT);
	char panic_reason[PANIC_REASON_LEN] = "Reason(s): ";
	u8   is_panic_reson_identified = 0;
	/*
	...
	case FW_ASSERT:
	{
	u32 fwfile_namehash = MEMIO_READ_FIELD(msg, FW_ASSERT_FILE_NAME_HASH);
	u32 fwfile_line = MEMIO_READ_FIELD(msg, FW_ASSERT_FILE_LINE);
	...
	case FW_SO:
	{
	u32 task_name = MEMIO_READ_FIELD(msg, FW_SO_TASK_NAME);
	char sztaskname[sizeof(u32) + 1];
...
static int vdeckm_handle_hosttomtx_msg(u32 *msg, struct lst_t *pend_pict_list,
				       enum vxd_msg_attr *msg_attr,
				       struct dec_decpict  **decpict,
				       u8 msg_type, u32 trans_id,
				       u32 msg_flags)
...
static int vdeckm_process_msg(const void *hndl_vxd, u32 *msg,
			      struct lst_t *pend_pict_list, u32 msg_flags,
			      enum vxd_msg_attr *msg_attr,
			      struct dec_decpict  **decpict)
{
	struct vdeckm_context *core_ctx = (struct vdeckm_context *)hndl_vxd;
	...
	*decpict = NULL;
	...
	default: {
		...
		pr_warn("Unknown message type received: 0x%x",
			msg_type);
...
static void vdeckm_vlr_copy(void *dst, void *src, u32 size)
{
	u32 *pdst = (u32 *)dst;
	u32 *psrc = (u32 *)src;
...
static int vdeckm_get_core_state(const void *hndl_vxd, struct vxd_states *state)
{
	...
	if (core_ctx->state_size == 0) {
		 ...
		 */
		regval = *((u32 *)core_ctx->comms_ram_addr +
			(PVDEC_COM_RAM_STATE_BUF_SIZE_AND_OFFSET_OFFSET / sizeof(u32)));
		core_ctx->state_size =
				...
				, u8 **msg)
{
	u8 vdec_flags = 0;
	u16 flags = 0;
	u8 *pmsg = kzalloc(FW_DEVA_DECODE_SIZE, GFP_KERNEL);
	struct vxdio_ddbufinfo *pbatch_msg_bufinfo =
	 ...
	 */
	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_GENMSG_MSG_TYPE, FW_DEVA_PARSE);

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_CTRL_ALLOC_ADDR,
			  (u32)pbatch_msg_bufinfo->dev_virt);

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_BUFFER_SIZE,
			  batch_msgdata->ctrl_alloc_bytes / sizeof(u32));

	/*
	 * Operating mode and decode flags
	 */
	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_OPERATING_MODE,
			  batch_msgdata->operating_mode);

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_FLAGS, flags);

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_VDEC_FLAGS, vdec_flags);

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_GENC_ID, batch_msgdata->genc_id);

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_MB_LOAD, batch_msgdata->mb_load);

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_STREAMID,
			  GET_STREAM_ID(batch_msgdata->transaction_id));

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_EXT_STATE_BUFFER,
			  (u32)batch_msgdata->pvdec_fwctx->dev_virt);

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_MSG_ID,
			  ++core_ctx->current_msgid);

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_TRANS_ID,
			  batch_msgdata->transaction_id);

	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_TILE_CFG,
			  batch_msgdata->tile_cfg);

	/*
	 * size of message
	 */
	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_GENMSG_MSG_SIZE,
			  FW_DEVA_DECODE_SIZE / sizeof(u32));
...
static int vdeckm_prepare_fragment(struct vdeckm_context *core_ctx,
				   ...
				   *fragment_msgdata,
				   u8 **msg)
{
	struct vxdio_ddbufinfo *pbatch_msg_bufinfo = NULL;
	u8 *pmsg = NULL;
	...
	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_FRAGMENT_CTRL_ALLOC_ADDR,
			  (u32)pbatch_msg_bufinfo->dev_virt
			  + fragment_msgdata->ctrl_alloc_offset);
	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_DECODE_FRAGMENT_BUFFER_SIZE,
			  fragment_msgdata->ctrl_alloc_bytes / sizeof(u32));

	/*
	 * size of message
	 */
	MEMIO_WRITE_FIELD(pmsg, FW_DEVA_GENMSG_MSG_SIZE,
			  FW_DEVA_DECODE_FRAGMENT_SIZE / sizeof(u32));
...
static int vdeckm_get_message(const void *hndl_vxd,
			      ...
			      struct hwctrl_to_kernel_msg *to_kernelmsg)
{
	...
	if (result == 0) {
		...
		MEMIO_READ_FIELD(to_kernelmsg->msg_hdr,
				 FW_DEVA_GENMSG_MSG_SIZE) * sizeof(u32);
	}
...
static u32 hwctrl_calculate_load(struct bspp_pict_hdr_info *pict_hdr_info)
{
...
int hwctrl_process_msg(void *hndl_hwctx, u32 msg_flags, u32 *msg,
		       struct dec_decpict **decpict)
{
	int result;
	...
	*decpict = NULL;
	...
	if (pdecpict) {
		if ((pdecpict->first_fld_fwmsg && val_first) ||
			...
			pr_err("device signalled critical error!!!\n");
		...
		if (msg_attr == VXD_MSG_ATTR_DECODED) {
			...
			if (!(pdecpict->second_fld_fwmsg)) {
				pr_err("No pending picture found\n");
				return IMG_ERROR_FATAL;
			}
			...
			if (result == 0) {
				...
				memcpy(&hwctx->prev_state, &state,
				       sizeof(struct hwctrl_state));

				for (pipe_minus1 = 0;
				     pipe_minus1 < hwctx->num_pipes;
				     pipe_minus1++) {
					hwctrl_dump_state(&state.core_state,
							  &state.core_state,
							  pipe_minus1);
				}
			}
...
{
	struct hwctrl_ctx *hwctx = (struct hwctrl_ctx *)hndl_hwctx;
	u32 result = IMG_ERROR_GENERIC_FAILURE;

	if (!hwctx->is_fatal_state && state) {
		struct vxd_states corestate;
		struct vxd_states *pcorestate = NULL;

		if (!state)
			pcorestate = &corestate;
		else
			...
			void **hndl_vdeckm_context)
{
	...
	result = vxd_get_coreproperties(&hndl_core_props, &corectx->props);
