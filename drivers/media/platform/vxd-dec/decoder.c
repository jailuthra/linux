...
static inline u32 get_next_picture_id(u32 cur_pict_id)
{
	return(cur_pict_id == FWIF_BIT_MASK(FWIF_NUMBITS_STREAM_PICTURE_ID) ?
	       1 : cur_pict_id + 1);
}

static inline u32 get_prev_picture_id(u32 cur_pict_id)
{
				  ...
				  u32 *num_pipes)
{
	...
	u32 ret;
	u32 ptd_align = DEV_MMU_PAGE_ALIGNMENT;
 ...
 */
int decoder_initialise(void *user_init_data, u32 int_heap_id,
...
error:
	if (*dec_ctx_handle) {
		if (dec_context->inited) {
			...
			deinit_result = decoder_deinitialise(*dec_ctx_handle);
			if (deinit_result != IMG_SUCCESS)
				pr_warn("decoder_deinitialise() failed to tidy-up after error");
		} else {
 ...
 */
static int decoder_stream_decode_resource_destroy(void *item,
						  void *free_cb_param)
{
 ...
 */
static int decoder_stream_reference_resource_destroy(void *item,
						     void *free_cb_param)
{
 ...
 */
int decoder_stream_destroy(void *dec_str_context, u8 abort)
{
	struct dec_str_ctx *dec_str_ctx_local;
...
decoder_stream_decode_resource_create(struct dec_str_ctx *dec_str_context)
{
	...
	mem_attribs = SYS_MEMATTRIB_UNCACHED | SYS_MEMATTRIB_WRITECOMBINE;
	mem_attribs |= SYS_MEMATTRIB_INTERNAL;
	...
	case VDEC_STD_H264:
		...
		ret = mmu_stream_alloc(dec_str_context->mmu_str_handle,
				       MMU_HEAP_STREAM_BUFFERS, mem_heap_id,
				       mem_attribs | SYS_MEMATTRIB_CPU_WRITE,
...
int decoder_stream_create(void *dec_ctx_arg,
			  ...
			  u32 km_str_id, void **mmu_str_handle,
 ...
 */
static struct dec_decoded_pict *decoder_get_decoded_pict(u32 transaction_id,
 ...
 */
static
struct dec_decoded_pict *decoder_get_decoded_pict_of_stream(u32 pict_id,
							    struct dq_linkage_t *dq_list)
{
	struct dec_decoded_pict *decoded_pict;
	void *item = NULL;
				      ...
				      u8 discard_refs)
{
	...
	int min_cnt;
	u32 min_cnt_idx;
	u32 num_display_pics = 0;
	u32 num_pics_displayed = 0;
	struct dec_decoded_pict *display_pict = NULL;
	u32 last_disp_pict_tid;
	u32 pict_id;
	...
	while (found) {
		...
		if (found) {
			u32 umin_cnt_tid = ctx_data->dpb[min_cnt_idx].transaction_id;
...
int decoder_stream_flush(void *dec_str_ctx_arg, u8 discard_refs)
{
	struct dec_str_ctx *dec_str_ctx;
 ...
 */
int decoder_stream_prepare_ctx(void *dec_str_ctx_arg, u8 flush_dpb)
{
	struct dec_str_ctx *dec_str_ctx =
 ...
 */
int decoder_get_load(void *dec_str_ctx_arg, u32 *avail_slots)
{
	struct dec_str_ctx *dec_str_ctx =
			decoder_stream_get_context(dec_str_ctx_arg);
		       ...
		       struct dec_decpict **dec_pict_ptr)
{
	...
	picture->dec_pict_info = kzalloc(sizeof(*picture->dec_pict_info),
					 GFP_KERNEL);
	...
	dec_pict->first_fld_fwmsg = kzalloc(sizeof(*dec_pict->first_fld_fwmsg),
					    GFP_KERNEL);
...
decoder_stream_reference_resource_create(struct dec_str_ctx *dec_str_ctx)
{
	...
	if (ret != IMG_SUCCESS) {
		pr_err("[USERSID=0x%08X] Failed to add resource",
		       dec_str_ctx->config.user_str_id);
		return ret;
	}
...
err_out_of_memory:
	if (pict_ref_res) {
		if (fw_ctrlbuf_allocated)
			mmu_free_mem(dec_str_ctx->mmu_str_handle,
				     &pict_ref_res->fw_ctrlbuf);
...
static int decoder_submit_fragment(struct dec_str_ctx *dec_str_context,
				   struct vdecdd_str_unit *str_unit, bool eop)
{
	struct dec_core_ctx *dec_core_context =
				decoder_str_ctx_to_core_ctx(dec_str_context);
 ...
 */
static u8 decoder_is_supported_by_atleast_onepipe(u8 *features, u32 num_pipes)
{
	u32 i;
		      ...
		      u32 *features)
{
	...
	if (disp_pict_rendinfo) {
		u32 stride_alignment = VDEC_VXD_EXT_STRIDE_ALIGNMENT_DEFAULT;
	...
	if (pict_hdrinfo && comseq_hdrinfo) {
		...
		u32 pict_size_in_mbs;
		u32 max_height = core_props->vidstd_props[str_cfg->vid_std].max_height;
		u32 max_width = core_props->vidstd_props[str_cfg->vid_std].max_width;
		u32 max_mbs = core_props->vidstd_props[str_cfg->vid_std].max_macroblocks;

		coded_cmd_width =
			ALIGN(pict_hdrinfo->coded_frame_size.width,
			      VDEC_MB_DIMENSION);
		coded_cmd_height =
			...
			struct vdecdd_str_unit *str_unit)
{
	...
	if (buf_control->second_field_of_pair) {
		...
		if (prev_decoded_pict) {
			...
			pr_warn("[USERSID=0x%08X] [TID 0x%08X] Failed to find decoded picture to attatch second_fld_tag_container",
				dec_str_ctx->config.user_str_id,
				decoded_pict->transaction_id);
		}
	...
	for (next_decoded_pict = decoder_next_picture(decoded_pict,
		...
		u8 last_to_display_for_seq = false;
		...
		if (!decoded_pict->force_display) {
			...
			if (buf_ctrl->second_field_of_pair) {
				...
				if (prev_decoded_pict) {
					...
					prev_decoded_pict->pict->dec_pict_sup_data.merged_flds = true;
					/* Mark the picture that was merged to the previous one. */
					decoded_pict->merged = true;
				} else {
					pr_warn("[USERSID=0x%08X] [TID 0x%08X] Failed to find decoded picture to attatch tag",
						dec_str_ctx->config.user_str_id,
						decoded_pict->transaction_id);
				}
			} else {
			...
			for (i = decoded_pict->rel_idx;
				...
				if (release_pict) {
					 ...
					 * come here.
					 */
					pr_err("[USERSID=0x%08X] Failed to find decoded picture [TID = 0x%08X] to release",
				      ...
				      u32 msg_size, u32 msg_flags)
{
	 ...
	 */
	ret = hwctrl_removefrom_piclist(dec_core_ctx->hw_ctx, dec_pict);
	VDEC_ASSERT(dec_pict);
	VDEC_ASSERT(ret == IMG_SUCCESS);
	if (ret != IMG_SUCCESS || !dec_pict)
		return IMG_ERROR_GENERIC_FAILURE;
	...
	while (dec_str_unit && !pict_start) {
		...
		pr_err("[USERSID=0x%08X] [TID=0x%08X] DECODED",
		       dec_str_ctx->config.user_str_id,
		       dec_pict->transaction_id);
 ...
 */
u8 decoder_is_stream_idle(void *dec_str_ctx_handle)
{
	struct dec_str_ctx *dec_str_ctx;
