...
struct bspp_grp_bstr_ctx {
	...
	s32 not_pic_unit_yet;
	s32 not_ext_pic_unit_yet;
	u32 total_data_size;
	...
	u32 segment_offset;
	s32 insert_start_code;
	u8 start_code_suffix;
	u8 current_view_idx;
};
 ...
 */
struct bspp_pps_info *bspp_get_pps_hdr(void *str_res_handle, u32 pps_id)
{
 ...
 */
static s32 bspp_free_bitstream_elem(struct bspp_bitstream_buffer *bstr_buf)
{
 ...
 *		it to the list.
 */
static s32 bspp_create_segment(struct bspp_grp_bstr_ctx *grp_btsr_ctx,
			       struct bspp_bitstream_buffer *cur_buf)
{
	...
	if (cur_buf->bytes_read != grp_btsr_ctx->segment_offset) {
		...
		segment->data_size = (u32)cur_buf->bytes_read
		 ...
		 */
		grp_btsr_ctx->segment_offset = (u32)cur_buf->bytes_read;

		if (grp_btsr_ctx->insert_start_code) {
			segment->bstr_seg_flag |= VDECDD_BSSEG_INSERT_STARTCODE;
			segment->start_code_suffix =
				grp_btsr_ctx->start_code_suffix;
 ...
 *
 */
static s32 bspp_determine_unit_type(enum vdec_vid_std vid_std, u8 unit_type,
				    s32 disable_mvc,
				    enum bspp_unit_type *unit_type_enum)
{
	/* Determine the unit type from the NAL type. */
	if (vid_std < VDEC_STD_MAX &&
	    parser_fxns[vid_std].determine_unit_type) {
		parser_fxns[vid_std].determine_unit_type(unit_type,
							 disable_mvc,
							 unit_type_enum);
	} else {
...
static void bspp_shift_reg_cb(enum swsr_cbevent event,
			      ...
			      u8 nal_type, u8 **data_buffer, u64 *data_size)
{
	u32 result;
	...
	case SWSR_EVENT_OUTPUT_BUFFER_END: {
		...
		if (cur_buf) {
			...
			grp_btsr_ctx->total_bytes_read +=
				(u32)cur_buf->bytes_read;
			...
			result = bspp_free_bitstream_elem(cur_buf);
			if (result != IMG_SUCCESS)
				goto error;
		} else {
			goto error;
	...
	case SWSR_EVENT_DELIMITER_NAL_TYPE:
		 ...
		 * list.
		 */
		result = bspp_determine_unit_type(grp_btsr_ctx->vid_std,
						  nal_type,
		...
		if (grp_btsr_ctx->last_unit_type != grp_btsr_ctx->unit_type &&
			...
			prev_pict_data = (grp_btsr_ctx->last_unit_type ==
					  BSPP_UNIT_PICTURE ||
					  grp_btsr_ctx->last_unit_type ==
					  BSPP_UNIT_SKIP_PICTURE) ? 1 : 0;

			curr_pict_data = (grp_btsr_ctx->unit_type ==
					  BSPP_UNIT_PICTURE ||
					  grp_btsr_ctx->unit_type ==
					  BSPP_UNIT_SKIP_PICTURE) ? 1 : 0;
			...
			if ((prev_pict_data && !curr_pict_data) ||
				 ...
				 * segment list.
				 */
				if (grp_btsr_ctx->last_unit_type !=
				    BSPP_UNIT_NONE &&
				    grp_btsr_ctx->segment_list !=
				    grp_btsr_ctx->pict_seg_list[2]) {
					struct bspp_bitstream_buffer *cur_buf =
					...
					/*
					 * Update the offset within current
					 * buf.
					 */
					 ...
					 */
					result =
						bspp_create_segment(grp_btsr_ctx,
								    cur_buf);
					if (result != IMG_SUCCESS)
						goto error;
				}
 ...
 *
 */
static void bspp_exception_handler(enum swsr_exception exception,
				   void *parse_ctx_handle)
{
	struct bspp_parse_ctx *parse_ctx =
		(struct bspp_parse_ctx *)parse_ctx_handle;
				...
				struct bspp_sequence_hdr_info *sequ_hdr_info)
{
	...
	struct bspp_ddbuf_array_info aux_fw_sequence =
		sequ_hdr_info->fw_sequence;
	void *aux_secure_sequence_info_hndl =
		sequ_hdr_info->secure_sequence_info;

	struct bspp_ddbuf_array_info *tmp = &sequ_hdr_info->fw_sequence;

	/* Reset all related structures. */
	memset(((u8 *)tmp->ddbuf_info.cpu_virt_addr + tmp->buf_offset),
	       0x00,
	...
	if (str_ctx->parser_callbacks.reset_data_cb)
		...
		memset(aux_secure_sequence_info_hndl, 0,
		       str_ctx->vid_std_features.seq_size);
	...
	sequ_hdr_info->sequ_hdr_info.bufmap_id =
		aux_fw_sequence.ddbuf_info.bufmap_id;
			   ...
			   struct bspp_pps_info *pps_info)
{
	...
	if (str_ctx->parser_callbacks.reset_data_cb) {
		str_ctx->parser_callbacks.reset_data_cb(BSPP_UNIT_PPS,
							pps_info->secure_pps_info);
	}
 ...
 *
 */
s32 bspp_stream_submit_buffer(void *str_context_handle,
			      const struct bspp_ddbuf_info *ddbuf_info,
			      u32 bufmap_id, u32 data_size,
			      void *pict_tag_param,
			      enum vdec_bstr_element_type bstr_element_type)
{
	struct bspp_str_context *str_ctx =
		(struct bspp_str_context *)str_context_handle;
	struct bspp_bitstream_buffer *bstr_buf;
	u32 result = IMG_SUCCESS;
 ...
 *
 */
s32 bspp_submit_picture_decoded(void *str_context_handle,
				struct bspp_picture_decoded *picture_decoded)
{
	...
	picture_decoded_elem =
		kmalloc(sizeof(struct bspp_picture_decoded),
			GFP_KERNEL);
 ...
 *
 */
static void bspp_check_and_detach_pps_info(struct bspp_stream_alloc_data *str_alloc, u32 pps_id)
{
	if (pps_id != BSPP_INVALID) {
		struct bspp_pps_info *pps_info =
			lst_first(&str_alloc->pps_data_list[pps_id]);
		...
		if (pps_info->ref_count == 0) {
			struct bspp_pps_info *next_pps_info =
				lst_next(pps_info);
			 ...
			 */
			if (next_pps_info) {
				lst_remove(&str_alloc->pps_data_list[pps_id],
					   pps_info);
				lst_addhead(&str_alloc->available_ppss_list,
					    pps_info);
			}
		}
	}
 ...
 *
 */
static s32 bspp_picture_decoded(struct bspp_str_context *str_ctx,
				struct bspp_picture_decoded *picture_decoded)
{
	struct bspp_stream_alloc_data *str_alloc = &str_ctx->str_alloc;
	...
	if (picture_decoded->sequ_hdr_id != BSPP_INVALID) {
		...
		/* If nothing references it any more */
		if (seq->ref_count == 0) {
			struct bspp_sequence_hdr_info *next_sequ_hdr_info =
				lst_next(seq);
			...
			if (next_sequ_hdr_info) {
				lst_remove(&str_alloc->sequence_data_list[picture_decoded->sequ_hdr_id],
					   seq);
				/* Release sequence data. */
				if (str_ctx->parser_callbacks.release_data_cb)
					str_ctx->parser_callbacks.release_data_cb((void *)str_alloc,
				...
				lst_addhead(&str_alloc->available_sequence_list,
					    seq);
			}
		}
	}
	...
	bspp_check_and_detach_pps_info(str_alloc,
				       picture_decoded->second_pps_id);
 ...
 *
 */
static s32 bspp_service_pictures_decoded(struct bspp_str_context *str_ctx)
{
	struct bspp_picture_decoded *picture_decoded;
	...
	while (1) {
		...
		picture_decoded =
			lst_removehead(&str_ctx->decoded_pictures_list);
...
static void bspp_remove_unused_sequence(struct bspp_str_context *str_ctx,
					u32 sps_id)
{
	struct bspp_stream_alloc_data *str_alloc = &str_ctx->str_alloc;
	 ...
	 */
	next_seq =
		lst_first(&str_alloc->sequence_data_list[sps_id]);
	while (next_seq) {
		/* Set Temp, it is the one which we will potentially remove */
		seq = next_seq;
		/*
		...
		if (seq->ref_count == 0 &&
			...
			lst_remove(&str_alloc->sequence_data_list[sps_id],
				   seq);
			if (str_ctx->parser_callbacks.release_data_cb) {
				str_ctx->parser_callbacks.release_data_cb((void *)str_alloc,
									  BSPP_UNIT_SEQUENCE,
 ...
 *
 */
static s32 bspp_return_or_store_sequence_hdr(struct bspp_str_context *str_ctx,
					     ...
					     struct bspp_sequence_hdr_info *sequ_hdr_info)
{
	...
	if (((parse_error & BSPP_ERROR_UNRECOVERABLE) ||
	     (parse_error & BSPP_ERROR_UNSUPPORTED)) &&
	    sequ_hdr_info->sequ_hdr_info.sequ_hdr_id != BSPP_INVALID) {
		prev_sequ_hdr_info =
			lst_last(&str_alloc->sequence_data_list[sequ_hdr_info->sequ_hdr_info.sequ_hdr_id]);

		/* check if it's not the same pointer */
		if (prev_sequ_hdr_info && prev_sequ_hdr_info != sequ_hdr_info) {
			/*
			 * Throw away corrupted sequence header if a previous
			 * "good" one exists.
			 */
			sequ_hdr_info->sequ_hdr_info.sequ_hdr_id = BSPP_INVALID;
		}
	...
	if (sequ_hdr_info->sequ_hdr_info.sequ_hdr_id != BSPP_INVALID) {
		...
		if (sequ_hdr_info !=
			 ...
			 * of the slot-list.
			 */
			lst_add(&str_alloc->sequence_data_list[sequ_hdr_info->sequ_hdr_info.sequ_hdr_id],
				sequ_hdr_info);
		}

		bspp_remove_unused_sequence(str_ctx,
 ...
 *
 */
static s32 bspp_get_resource(struct bspp_str_context *str_ctx,
			     struct bspp_pict_hdr_info *pict_hdr_info,
			     struct bspp_unit_data *unit_data)
{
	s32 result = IMG_SUCCESS;
	struct bspp_stream_alloc_data *str_alloc = &str_ctx->str_alloc;
	...
	case BSPP_UNIT_PPS:
		...
		unit_data->out.pps_info =
			lst_removehead(&str_alloc->available_ppss_list);
 ...
 */
static s32 bspp_file_resource(struct bspp_str_context *str_ctx,
			      struct bspp_unit_data *unit_data)
{
	u32 result;
	struct bspp_stream_alloc_data *str_alloc = &str_ctx->str_alloc;
	...
	case BSPP_UNIT_PPS:
		...
		if (unit_data->out.pps_info->pps_id != BSPP_INVALID) {
			...
			bspp_remove_unused_pps(str_ctx,
					       unit_data->out.pps_info->pps_id);
		} else {
			/*
 ...
 *
 */
static s32 bspp_process_unit(struct bspp_str_context *str_ctx,
			     u32 size_delim_bits,
			     struct bspp_pict_ctx *pict_ctx,
			     struct bspp_parse_state *parse_state)
{
	struct bspp_unit_data unit_data;
	u64 unit_size = 0; /* Unit size (in bytes, size delimited only). */
	u32 result;
	u8 vidx = str_ctx->grp_bstr_ctx.current_view_idx;
	...
	if (str_ctx->grp_bstr_ctx.delim_present) {
		 ...
		 */
		result =
			swsr_consume_delim(str_ctx->swsr_ctx.swsr_context,
	 ...
	 */
	result =
		str_ctx->parser_callbacks.parse_unit_cb(str_ctx->swsr_ctx.swsr_context,
							&unit_data);
	if (result != IMG_SUCCESS) {
		pr_err("Failed to process unit, error = %d",
		       unit_data.parse_error);
		goto error;
	}
	...
	if (str_ctx->grp_bstr_ctx.unit_type == BSPP_UNIT_PICTURE ||
		...
		if (unit_data.ext_slice && str_ctx->grp_bstr_ctx.not_ext_pic_unit_yet &&
			...
			u32 id = unit_data.pict_sequ_hdr_id;
		...
		if (unit_data.slice) {
			if (str_ctx->grp_bstr_ctx.not_pic_unit_yet &&
				...
				if (str_ctx->swsr_ctx.sr_config.delim_type ==
					...
					/* VDECFW_SCP_ONLY; */
					curr_pict_hdr_info->parser_mode = 0;
			}
...
error:
...

	{
		 ...
		 * Store or return resource used for parsing unit.
		 */
		new_result = bspp_file_resource(str_ctx, &unit_data);
	}
 ...
 *
 */
static s32 bspp_terminate_buffer(struct bspp_grp_bstr_ctx *grp_btsr_ctx,
				 struct bspp_bitstream_buffer *buf)
{
	...
	result = bspp_free_bitstream_elem(buf);
	if (result != IMG_SUCCESS)
		goto error;
 ...
 *
 */
static s32 bspp_jump_to_next_view(struct bspp_grp_bstr_ctx *grp_btsr_ctx,
				  ...
				  struct bspp_parse_state *parse_state)
{
	...
	s32 result;
	u32 i;
	u8 vidx = grp_btsr_ctx->current_view_idx;
	...
	if (cur_buf->bufmap_id != parse_state->prev_buf_map_id) {
		...
		if (segment && segment->bufmap_id ==
				parse_state->prev_buf_map_id) {
			...
			memset(&prev_buf, 0,
			       sizeof(struct bspp_bitstream_buffer));
...
static void bspp_reset_pict_state(struct bspp_str_context *str_ctx,
				  struct bspp_pict_ctx *pict_ctx,
				  struct bspp_parse_state *parse_state)
{
	...
	parse_state->prev_bottom_pic_flag = (u8)BSPP_INVALID;
 ...
 */
s32 bspp_stream_preparse_buffers(void *str_context_handle,
				 const struct bspp_ddbuf_info *contig_buf_info,
				 u32 contig_buf_map_id, struct lst_t *segments,
				 struct bspp_preparsed_data *preparsed_data,
				 s32 end_of_pic)
{
	struct bspp_str_context *str_ctx =
	...
	u32 i;
	u32 unit_count = 0, num_arrays = 0;
	u32 size_delim_bits = 0;
	...
	if (str_ctx->parser_callbacks.initialise_parsing_cb &&
	    parse_state->first_chunk) {
	...
	for (i = 0; i < VDEC_H264_MVC_MAX_VIEWS; i++) {
		...
		pict_ctx->pict_hdr_info[i].second_pict_aux_data.id =
			BSPP_INVALID;
	}
	...
	str_ctx->grp_bstr_ctx.segment_list =
		str_ctx->grp_bstr_ctx.pre_pict_seg_list[0];
	str_ctx->grp_bstr_ctx.pict_tag_param =
		str_ctx->grp_bstr_ctx.pict_tag_param_array[0];
	 ...
	 */
	if (pict_ctx->sequ_hdr_info &&
	    pict_ctx->sequ_hdr_info->ref_count == 0) {
		pict_ctx->present = 0;
		pict_ctx->finished = 1;
	}
	...
	if (str_ctx->swsr_ctx.sr_config.delim_type == SWSR_DELIM_SIZE) {
		...
		if (cur_buf->bstr_element_type ==
		    VDEC_BSTRELEMENT_CODEC_CONFIG &&
		    str_ctx->parser_callbacks.parse_codec_config_cb) {
			/* Parse codec config header and catch any exceptions */
			str_ctx->parser_callbacks.parse_codec_config_cb(str_ctx->swsr_ctx.swsr_context,
									&unit_count,
									&num_arrays,
	...
	while (found != SWSR_FOUND_EOD && !pict_ctx->finished) {
		...
		if (str_ctx->swsr_ctx.sr_config.delim_type ==
			 ...
			 * any exceptions.
			 */
			str_ctx->parser_callbacks.update_unit_counts_cb(str_ctx->swsr_ctx.swsr_context,
									&unit_count,
									&num_arrays);
		}

		/* Process the next unit. */
		result = bspp_process_unit(str_ctx, size_delim_bits, pict_ctx,
					   parse_state);
		...
		if (!pict_ctx->finished) {
...

			{
				...
				if (buf) {
					parse_state->prev_buf_map_id =
						buf->bufmap_id;
					parse_state->prev_buf_data_size =
						buf->data_size;
				}
			}
		}
	...
	if (pict_ctx->present && !pict_ctx->invalid) {
		if (!pict_ctx->new_pict_signalled) {
			...
			if (pict_ctx->ext_sequ_hdr_info) {
				...
				preparsed_data->ext_pictures_data[0].pict_hdr_info.first_pic_of_sequence =
		...
		while (segment) {
			lst_add(&preparsed_data->picture_data.pict_seg_list[0],
				segment);
			segment =
				lst_removehead(&str_ctx->inter_pict_data.pic_prefix_seg);
		}
		...
		for (i = 0; i < VDEC_H264_MVC_MAX_VIEWS; i++) {
			...
			if (preparsed_data->ext_pictures_data[i].is_prefix) {
				for (j = 0; j < BSPP_MAX_PICTURES_PER_BUFFER;
				     j++) {
					segment =
						lst_removehead(&ext_pic_data->pict_seg_list[j]);
					while (segment) {
 ...
 *
 */
s32 bspp_stream_destroy(void *str_context_handle)
{
	struct bspp_str_context *str_ctx =
		(struct bspp_str_context *)str_context_handle;
	u32 i;
	...
	for (i = 0; i < MAX_SEQUENCES; i++) {
		sequ_hdr_info =
			lst_removehead(&str_ctx->str_alloc.available_sequence_list);
		if (sequ_hdr_info &&
		    str_ctx->parser_callbacks.destroy_data_cb)
			str_ctx->parser_callbacks.destroy_data_cb(BSPP_UNIT_SEQUENCE,
								  sequ_hdr_info->secure_sequence_info);
	}
	...
	if (str_ctx->vid_std_features.uses_pps) {
		for (i = 0; i < MAX_PPSS; i++) {
			pps_info =
				lst_removehead(&str_ctx->str_alloc.available_ppss_list);
			if (pps_info &&
			    str_ctx->parser_callbacks.destroy_data_cb)
				str_ctx->parser_callbacks.destroy_data_cb(BSPP_UNIT_PPS,
									  pps_info->secure_pps_info);
		}
 ...
 *
 */
s32 bspp_set_codec_config(const void *str_context_handle,
 ...
 *
 */
s32 bspp_stream_create(const struct vdec_str_configdata *str_config_data,
		       void **str_ctx_handle,
		       ...
		       struct bspp_ddbuf_array_info fw_pps[])
{
	...
	str_ctx->intra_frame_closed_gop =
		str_config_data->intra_frame_closed_gop;
	...
	if (str_ctx->vid_std < VDEC_STD_MAX &&
	    parser_fxns[str_ctx->vid_std].set_parser_config) {
	...
	str_ctx->sequ_hdr_info =
		kmalloc((MAX_SEQUENCES * sizeof(struct bspp_sequence_hdr_info)),
			GFP_KERNEL);
	...
	sequ_hdr_info =
		(struct bspp_sequence_hdr_info *)(str_ctx->sequ_hdr_info);
	for (i = 0; i < MAX_SEQUENCES; i++) {
		...
		sequ_hdr_info->secure_sequence_info =
			(void *)(str_ctx->secure_sequence_info +
	...
	if (str_ctx->vid_std_features.uses_pps) {
		...
		str_ctx->pps_info =
			kmalloc((MAX_PPSS * sizeof(struct bspp_pps_info)),
				GFP_KERNEL);
		if (!str_ctx->pps_info) {
			result = IMG_ERROR_OUT_OF_MEMORY;
			goto error;
		}
		memset(str_ctx->pps_info, 0x00,
		       (MAX_PPSS * sizeof(struct bspp_pps_info)));
		str_ctx->secure_pps_info =
			kmalloc((MAX_PPSS * str_ctx->vid_std_features.pps_size),
				GFP_KERNEL);
		...
		for (i = 0; i < MAX_PPSS; i++) {
			...
			/*
			 * We have no container for the PPS that passes down to the kernel, for this reason the
			 * h264 secure parser needs to populate that info into the picture header (Second)PictAuxData.
			 */
			pps_info->secure_pps_info =
				(void *)(str_ctx->secure_pps_info +
					 (i *
		...
		/* As only standards that use PPS also use VUI, initialise the appropriate data structures here. */
