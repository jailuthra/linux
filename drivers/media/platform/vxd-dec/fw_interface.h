...
struct ctrl_alloc_header {
	u32 cmd_additional_params;
	u32 slice_params;
	union {
		u32 vp8_probability_data;
		u32 h264_pipeintra_buffersize;
	};
	u32 chroma_strides;
	u32 slice_first_mb_yx;
	u32 pic_last_mb_yx;
	/* VC1 only : Store Range Map flags in bottom bits of [0] */
	u32 alt_output_addr[2];
	u32 alt_output_flags;
	/* H264 Only : Extended Operating Mode */
	u32 ext_opmode;
};

#define CMD_CTRL_ALLOC_HEADER_DWSIZE \
			(sizeof(struct ctrl_alloc_header) / sizeof(u32))
...
#define CMD_DEBLOCK_DWSIZE		(sizeof(DEBLOCK_CMD) / sizeof(u32))
#endif /* !VDEC_USE_PVDEC_SEC */
...
#define CMD_DMA_DWSIZE				(sizeof(DMA_CMD) / sizeof(u32))
...
#define CMD_DMA_OFFSET_DWSIZE	(sizeof(DMA_CMD_WITH_OFFSET) / sizeof(u32))
...
struct host_copy_cmd {
	u32 cmd;
	u32 src_dev_virt_add;
	u32 dst_dev_virt_add;
};

#define CMD_HOST_COPY_DWSIZE		(sizeof(HOST_COPY_CMD) / sizeof(u32))
...
struct sr_setup_cmd {
	u32 cmd;
	u32 bitstream_offset_bits;
	u32 bitstream_size_bytes;
};

#define CMD_SR_DWSIZE			(sizeof(SR_SETUP_CMD) / sizeof(u32))
...
#define CMD_PARSE_DWSIZE		(sizeof(PARSE_HEADER_CMD) / sizeof(u32))
...
struct vdec_ext_cmd {
	...
	u32	mem_to_reg_size;
	u32	slice_params_addr;
	u32	slice_params_size; /* dwords */
	u32	last_luma_recon;
	u32	last_chroma_recon;
	u32	luma_err_base;
	u32	chroma_err_base;
	u32	scaled_display_size;
	u32	horz_scale_control;
	u32	vert_scale_control;
	u32	scale_output_size;
	u32	vlc_idx_table_size;
	u32	vlc_idx_table_addr;
	u32	vlc_tables_size;
	u32	vlc_tables_addr;
	u32	display_picture_size;
	u32	parser_mode;
	/* needed for separate colour planes */
	u32	intra_buf_base_addr;
	u32	intra_buf_size_per_plane;
	u32	intra_buf_size_per_pipe;
	u32	chroma2reconstructed_addr;
	u32	luma_alt_addr;
	u32	chroma_alt_addr;
	u32	chroma2alt_addr;
	u32	aux_line_buf_size_per_pipe;
	u32	aux_line_buffer_base_addr;
	u32	alt_output_pict_rotation;
	/* miscellaneous flags */
	struct {
...
#define CMD_VDEC_EXT_DWSIZE		(sizeof(VDEC_EXT_CMD) / sizeof(u32))
#endif /* VDEC_USE_PVDEC_COMPATIBILITY */
