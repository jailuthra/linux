...
enum img_pixfmt pixel_get_pixfmt(enum pixel_fmt_idc chroma_fmt_idc,
				 ...
				 u32 num_planes)
{
	u32 internal_num_planes = (num_planes == 0 || num_planes > 4) ? 2 :
								num_planes;
...
static int pixel_get_internal_pixelinfo(struct pixel_pixinfo *pixinfo,
...
static int pixel_yuv_get_descriptor_int(struct pixel_info *pixinfo,
		       ...
		       struct img_pixfmt_desc *pix_desc)
{
	...
	ret = pixel_get_internal_pixelinfo(pix_info, &int_pix_info);
	...
	ret = pixel_yuv_get_descriptor_int(&int_pix_info, pix_desc);
	VDEC_ASSERT(ret == IMG_SUCCESS);
	if (ret != IMG_SUCCESS)
		return ret;
...
int pixel_get_fmt_desc(enum img_pixfmt pix_fmt,
		       struct img_pixfmt_desc *pix_desc)
{
	if (pix_fmt >= IMG_PIXFMT_ARBPLANAR8 &&
		...
		for (i = 0; i < FACT_SPEC_FORMAT_NUM_PLANES; i++) {
			u8 code = (spec >> FACT_SPEC_FORMAT_PLANE_CODE_BITS *
