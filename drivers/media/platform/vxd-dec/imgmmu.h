...
static const size_t VIRT_PAGE_OFF_MASK = ((1 << MMU_PAGE_SHIFT) - 1);
...
static const size_t VIRT_DIR_IDX_MASK = (~((1 << MMU_DIR_SHIFT) - 1));
 ...
 * @phys_addr: physical address - u64 is used to support extended physical
 *	       address on 32bit system
 * @cpu_virt_addr: CPU virtual address pointer
 */
struct mmu_page_cfg {
	u64 phys_addr;
	uintptr_t cpu_virt_addr;
};
...
typedef void (*mmu_pfn_page_write) (struct mmu_page_cfg *mmu_page,
				    unsigned int offset,
				    u64 pa_to_write, unsigned int flags);
 ...
 *			 MMU (in Bytes)
 */
static inline size_t mmu_get_page_size(void)
{
