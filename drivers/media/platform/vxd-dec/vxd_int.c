...
static u32 amsvdx_codecmode[VDEC_STD_MAX] = {
		/* Invalid */
		-1,
		/* MPEG2 */
		3,
...
struct msvdx_scaler_coeff_cmds {
	u32  acmd_horizluma_coeff[VDECFW_NUM_SCALE_COEFFS];
	u32  acmd_vertluma_coeff[VDECFW_NUM_SCALE_COEFFS];
	u32  acmd_horizchroma_coeff[VDECFW_NUM_SCALE_COEFFS];
	u32  acmd_vertchroma_coeff[VDECFW_NUM_SCALE_COEFFS];
};
...
static const u32  amsvdx_64byte_row_stride[] = {
...
static u32 maint_rev;

static u32 get_stride_code(enum vdec_vid_std vidstd, u32 row_stride)
{
	u32 i;
...
static u32 vxd_getprofile(enum vdec_vid_std vidstd, u32 std_profile)
{
	u32  profile = 0;
...
static int vxd_getcoreproperties(struct vxd_coreprops *coreprops,
				 u32 corerev,
				 u32 pvdec_coreid, u32 mmu_config0,
				 u32 mmu_config1, u32 *pixel_pipecfg,
				 u32 *pixel_misccfg, u32 max_framecfg)
{
	u32 group_id;
	u32 core_id;
	u32 core_config;
	u32 extended_address_range;
	u8  group_size;
	u8  pipe_minus1 = 0;
	u32 max_h264_hw_chromaformat = 0;
	u32 max_hevc_hw_chromaformat = 0;
	u32 max_bitdepth_luma = 0;
	u32 i;
	...
	core_rev.maj_rev = REGIO_READ_FIELD(corerev, PVDEC_CORE,
					    CR_PVDEC_CORE_REV,
					    CR_PVDEC_MAJOR_REV);
	core_rev.min_rev = REGIO_READ_FIELD(corerev, PVDEC_CORE,
					    CR_PVDEC_CORE_REV,
					    CR_PVDEC_MINOR_REV);
	core_rev.maint_rev = REGIO_READ_FIELD(corerev, PVDEC_CORE,
					      CR_PVDEC_CORE_REV,
					      CR_PVDEC_MAINT_REV);

	/* core id */
	group_id = REGIO_READ_FIELD(pvdec_coreid, PVDEC_CORE,
				    CR_PVDEC_CORE_ID, CR_GROUP_ID);
	core_id = REGIO_READ_FIELD(pvdec_coreid, PVDEC_CORE,
				   CR_PVDEC_CORE_ID, CR_CORE_ID);
	...
	coreprops->mmu_support_stride_per_context =
			REGIO_READ_FIELD(mmu_config1, IMG_VIDEO_BUS4_MMU,
					 MMU_CONFIG1,
					 SUPPORT_STRIDE_PER_CONTEXT) ==
					 1 ? 1 : 0;

	coreprops->mmu_support_secure =
			REGIO_READ_FIELD(mmu_config1, IMG_VIDEO_BUS4_MMU,
					 MMU_CONFIG1, SUPPORT_SECURE) ==
					 1 ? 1 : 0;

	extended_address_range = REGIO_READ_FIELD(mmu_config0,
						  IMG_VIDEO_BUS4_MMU,
						  MMU_CONFIG0,
						  EXTENDED_ADDR_RANGE);
	...
	for (pipe_minus1 = 0; pipe_minus1 < coreprops->num_pixel_pipes;
	     pipe_minus1++) {
		u32 current_bitdepth =
			GET_BITS(pixel_misccfg[pipe_minus1], 4, 3) + 8;
		u32 current_h264_hw_chromaformat =
			GET_BITS(pixel_misccfg[pipe_minus1], 0, 2);
		u32 current_hevc_hw_chromaformat =
			  ...
			  u32 *pict_cmds)
{
	 ...
	 * is not fulfilled.
	 */
	if (str_configdata->vid_std != VDEC_STD_VC1 &&
	    ((coded_height - disp_height) > 0x0F)) {
	...
	y_stride =
	buffers->recon_pict->rend_info.plane_info[VDEC_PLANE_VIDEO_Y].stride;
	uv_stride =
	buffers->recon_pict->rend_info.plane_info[VDEC_PLANE_VIDEO_UV].stride;
	v_stride =
	buffers->recon_pict->rend_info.plane_info[VDEC_PLANE_VIDEO_V].stride;
	...
	pict_cmds[VDECFW_CMD_LUMA_RECONSTRUCTED_PICTURE_BASE_ADDRESS] =
		(u32)GET_HOST_ADDR(&buffers->recon_pict->pict_buf->ddbuf_info) +
	...
	pict_cmds[VDECFW_CMD_INTRA_BUFFER_BASE_ADDRESS] =
		(u32)GET_HOST_ADDR(buffers->intra_bufinfo);
	...
	pict_cmds[VDECFW_CMD_AUX_LINE_BUFFER_BASE_ADDRESS] =
		(u32)GET_HOST_ADDR(buffers->auxline_bufinfo);
			...
			u32 *pict_cmds)
{
	...
	if (!output_config->force_oold &&
	    output_config->pixel_info.chroma_fmt_idc == chroma_fmt_idc &&
	    output_config->pixel_info.bitdepth_y ==
	    str_unit->seq_hdr_info->com_sequ_hdr_info.pixel_info.bitdepth_y &&
	    output_config->pixel_info.bitdepth_c ==
	    str_unit->seq_hdr_info->com_sequ_hdr_info.pixel_info.bitdepth_c) {
		/*
		 * When supplied with an alternate output buffer and not using
		 * it for transformation (e.g. scaling. rotating or
		 * up/down-sampling) signal to use the scaler as a means of
		 * generating output with an arbitrary stride.
		 */
		post_process =
		str_unit->seq_hdr_info->com_sequ_hdr_info.post_processing;
		if (str_configdata->vid_std == VDEC_STD_VC1) {
			if (post_process &&
			    !str_unit->pict_hdr_info->post_processing)
				buse_scaler_bypass = 1;
		} else {
			buse_scaler_bypass = 1;
		}
	}
	...
	y_stride =
	buffers->alt_pict->rend_info.plane_info[VDEC_PLANE_VIDEO_Y].stride;
	uv_stride =
	buffers->alt_pict->rend_info.plane_info[VDEC_PLANE_VIDEO_UV].stride;
	v_stride =
	buffers->alt_pict->rend_info.plane_info[VDEC_PLANE_VIDEO_V].stride;
	...
	REGIO_WRITE_FIELD(pict_cmds[VDECFW_CMD_ALTERNATIVE_OUTPUT_CONTROL],
			  ...
			  ALT_OUTPUT_FORMAT,
			  str_unit->seq_hdr_info->com_sequ_hdr_info.separate_chroma_planes ?
			  0 : pixel_get_hw_chroma_format_idc(output_config->pixel_info.chroma_fmt_idc));
	...
	pict_cmds[VDECFW_CMD_LUMA_ALTERNATIVE_PICTURE_BASE_ADDRESS] =
		(u32)GET_HOST_ADDR(&buffers->alt_pict->pict_buf->ddbuf_info) +
		      ...
		      u32 *pict_cmds)
{
	 ...
	 */
	bool bforce_luma_resampling = coreprops->hevc[0];
	...
	REGIO_WRITE_FIELD(pict_cmds[VDECFW_CMD_ALTERNATIVE_OUTPUT_CONTROL],
			  ...
			  (pitch->vert_luma == FIXED(1, HIGHP)));

	REGIO_WRITE_FIELD(pict_cmds[VDECFW_CMD_ALTERNATIVE_OUTPUT_CONTROL],
			  MSVDX_CMDS, ALTERNATIVE_OUTPUT_CONTROL,
			  ALT_MEMORY_PACKING, pixel_get_hw_memory_packing(out_loop_pixel_info->mem_pkg));
...
{
	return (u8)amsvdx_codecmode[vidstd];
}
			   ...
			   struct vxd_coreprops *vxd_coreprops)
{
	...
	vxd_getcoreproperties(vxd_coreprops,
			      props->core_rev,
			      props->pvdec_core_id,
			      props->mmu_config0,
			      props->mmu_config1,
