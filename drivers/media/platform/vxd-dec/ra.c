 ...
 */
static struct btag *ra_build_span_marker(u64 base)
{
 ...
 * @Return    btag *       : New boundary tag
 */
static struct btag *ra_build_bt(u64 base, u64 size)
{
	struct btag *local_bt = NULL;
	int res = IMG_ERROR_FATAL;
			      ...
			      u64 size)
{
	...
	if ((u64)1 << arena_arg->max_idx < size)
		arena_arg->max_idx++;
		       ...
		       struct btag *bt_arg)
{
	...
	if (bt_neibr &&
		...
		res = pool_free(global_pool_bt, bt_neibr);
	}
	...
	if (bt_arg->nxt_seg &&
		...
		arena_arg->import_free_fxn(arena_arg->import_hdnl,
					   bt_arg->base,
					   bt_arg->ref);
		res = pool_free(global_pool_bt, ps_bt_nxt);
		res = pool_free(global_pool_bt, ps_bt_prev);
		res = pool_free(global_pool_bt, bt_arg);
	} else {
 ...
 *
 */
int vid_ra_create(const char * const name,
		  u64 base_arg,
		  u64 size_arg,
		  size_t quantum,
					 ...
					 void **arena_hndl)
{
	...
	*(arena_hndl) = NULL;
	...
	if (global_init) {
		...
		if (name)
			local_arena->name = kstrdup((char *)name, GFP_KERNEL);
 ...
 *
 */
int vid_ra_add(void * const arena_hndl, u64 base_arg, u64 size_arg)
{
	int res = IMG_ERROR_FATAL;
	struct arena *local_arena = NULL;
		 ...
		 u64 * const basearg)
{
	...
	if (global_init) {
		...
		if (res != IMG_SUCCESS) {
			...
			u64 import_base = (u64)0;
			u64 locimprt_reqsz = loc_size;
			u64 locimprt_actsz = (u64)0;
 ...
 *
 */
int vid_ra_free(void * const arena_hndl, u64 base_arg)
{
	int res = IMG_ERROR_FATAL;
	struct arena *local_arena = NULL;
	struct btag *local_bt = NULL;
