 ...
 */
static inline unsigned int mmu_dir_entry(uintptr_t vaddr)
{
 ...
 * @vaddr: virtual address
 */
static inline unsigned int mmu_pg_entry(uintptr_t vaddr)
{
...
static void mmu_pg_wr(struct mmu_page_cfg *mmu_page, unsigned int offset,
		      u64 pa_to_write, unsigned int mmu_flag)
{
	u32 *dir_mem = NULL;
	u64 cur_pa = pa_to_write;

	if (!mmu_page)
		return;

	dir_mem = (u32 *)mmu_page->cpu_virt_addr;
	/*
	 ...
	 * flags so it's ok
	 */
	dir_mem[offset] = (u32)cur_pa | (mmu_flag);
}
 ...
 * @return ERR_PTR in case of an error and result has the value:
...
struct mmu_directory *mmu_create_directory(const struct mmu_info *mmu_info_ops)
{
	...
	neo->dir_page_table = kcalloc(MMU_N_TABLE,
					 ...
					 void *phys_iter_arg)
{
	...
	u32 i;
	u32 d;
	const u32 duplicate = PAGE_SIZE / mmu_get_page_size();
	int res = 0;
	 ...
	 */
	to_update = kcalloc(MMU_N_TABLE, sizeof(u32), GFP_KERNEL);
	if (!to_update)
		return ERR_PTR(-ENOMEM);
	...
	for (i = 0; i < n_entries * duplicate; i++) {
		...
		if (dir_pgtbl[dir_off]) {
			...
			u32 *page_mem = (u32 *)tbl->page->cpu_virt_addr;
	...
	if (mmu_dir->mmu_info_cfg.pfn_page_update) {
		while (n_pgs_to_update > 0) {
			u32 idx = to_update[n_pgs_to_update - 1];
...
static int sg_phys_iter_next(void *arg, u64 *next)
{
 ...
 * @return ERR_PTR in case of an error with the following values:
