...
static const unsigned long vxd_plat_poll_udelay = 100;
...
static void pvdec_prep_fw_upload(const struct device *dev,
				 ...
				 u8 dma_channel)
{
	u32 fw_vxd_virt_addr = ena_params->fw_buf_virt_addr;
	u32 vxd_ptd_addr = ena_params->ptd;
	u32 reg = 0;
	int i;
	 ...
	 */
	reg = 0;
	reg = VXD_WR_REG_FIELD(reg, IMG_VIDEO_BUS4_MMU, MMU_ADDRESS_CONTROL,
			       UPPER_ADDR_FIXED, 0);
	reg = VXD_WR_REG_FIELD(reg, IMG_VIDEO_BUS4_MMU, MMU_ADDRESS_CONTROL,
			       MMU_ENA_EXT_ADDR, 1);
	reg = VXD_WR_REG_FIELD(reg, IMG_VIDEO_BUS4_MMU, MMU_ADDRESS_CONTROL,
...
static int pvdec_send_pad_msg(void __iomem *reg_base)
{
	...
	wr_idx = 0;
	VXD_WR_REG_ABS(reg_base, VLR_OFFSET + PVDEC_FW_TO_MTX_WR_IDX_OFFSET,
		       wr_idx);
...
static void pvdec_select_pipe(void __iomem *reg_base, u8 pipe)
{
	u32 reg = 0;

	reg = VXD_WR_REG_FIELD(reg, PVDEC_CORE, PVDEC_HOST_PIPE_SELECT,
			       PIPE_SEL, pipe);
				 ...
				 struct vxd_ena_params *ena_params)
{
	...
	if (ena_params->mem_staller.data) {
		...
		if (size == PVDEC_CORE_MEMSTALLER_ELEMENTS) {
			...
			dev_warn(dev,
				 "%s: Wrong layout of mem staller config (%u)!",
				 __func__, size);
		}
	}
}
...
static void pvdec_post_boot_setup(const struct device *dev,
				  void __iomem *reg_base, u32 freq_khz)
{
	int reg;
	...
	VXD_WR_RPT_REG(reg_base, IMG_VIDEO_BUS4_MMU, MMU_TILE_MAX_ADDR,
		       PVDEC_HEAP_TILE512_START + PVDEC_HEAP_TILE512_SIZE - 1,
		       0);
	VXD_WR_RPT_REG(reg_base, IMG_VIDEO_BUS4_MMU, MMU_TILE_MIN_ADDR,
		       PVDEC_HEAP_TILE1024_START, 1);
	VXD_WR_RPT_REG(reg_base, IMG_VIDEO_BUS4_MMU, MMU_TILE_MAX_ADDR,
		       PVDEC_HEAP_TILE1024_START + PVDEC_HEAP_TILE1024_SIZE - 1,
		       1);
	VXD_WR_RPT_REG(reg_base, IMG_VIDEO_BUS4_MMU, MMU_TILE_MIN_ADDR,
		       PVDEC_HEAP_TILE2048_START, 2);
	VXD_WR_RPT_REG(reg_base, IMG_VIDEO_BUS4_MMU, MMU_TILE_MAX_ADDR,
		       PVDEC_HEAP_TILE2048_START + PVDEC_HEAP_TILE2048_SIZE - 1,
		       2);
	VXD_WR_RPT_REG(reg_base, IMG_VIDEO_BUS4_MMU, MMU_TILE_MIN_ADDR,
		       PVDEC_HEAP_TILE4096_START, 3);
	VXD_WR_RPT_REG(reg_base, IMG_VIDEO_BUS4_MMU, MMU_TILE_MAX_ADDR,
		       PVDEC_HEAP_TILE4096_START + PVDEC_HEAP_TILE4096_SIZE - 1,
		       3);
...
static void pvdec_clock_measure(void __iomem *reg_base,
				struct timespec *start_time, u32 *start_ticks)
{
	local_irq_disable();
				 ...
				 u32 *freq_khz)
{
	...
	if (span_nsec) {
		u64 res = (u64)tot_ticks * 1000000UL;
...
static int pvdec_wait_dma_done(const struct device *dev,
			       void __iomem *reg_base, size_t size,
			       u8 dma_channel)
{
	u32 reg, timeout = PVDEC_TIMEOUT_COUNTER, prev_count, count = size;
...
static int pvdec_start_fw_dma(const struct device *dev,
			      void __iomem *reg_base, u8 dma_channel,
			      size_t fw_buf_size, u32 *freq_khz)
{
	u32 reg = 0;
	int ret = 0;
	...
	reg = VXD_WR_REG_FIELD(reg, PVDEC_PIXEL, PIXEL_MAN_CLK_ENA,
	...
	reg = VXD_WR_REG_FIELD(reg, DMAC, DMAC_PERIPH, EXT_BURST,
			       DMAC_EXT_BURST_0);
...
static int pvdec_set_clocks(void __iomem *reg_base, u32 req_clocks)
{
	u32 clocks = 0, reg;
	u32 pvdec_timeout;
...
static void pvdec_ena_mtx_int(void __iomem *reg_base)
{
	...
	reg = VXD_WR_REG_FIELD(reg, PVDEC_CORE, PVDEC_INT_STAT,
			       HOST_PROC_IRQ, 1);
	reg = VXD_WR_REG_FIELD(reg, PVDEC_CORE, PVDEC_INT_STAT,
...
static int pvdec_reset(void __iomem *reg_base, bool skip_pipe_clocks)
{
	...
	reg = 0;
	reg = VXD_WR_REG_FIELD(reg, PVDEC_CORE, PVDEC_INT_CLEAR, IRQ_CLEAR,
			       0xFFFF);
	...
	reg = 0;
	reg = VXD_WR_REG_FIELD(reg, IMG_VIDEO_BUS4_MMU, MMU_CONTROL1,
			       MMU_PAUSE_SET, 1);
	...
	reg = 0;
	reg = VXD_WR_REG_FIELD(reg, PVDEC_CORE, PVDEC_SOFT_RST,
				...
				struct vxd_core_props *props)
{
	...
	{
		u32 fifo_ctrl = VXD_RD_REG(reg_base, PVDEC_CORE,
					   PROC_DBG_FIFO_CTRL0);
		       ...
		       struct vxd_dev *ctx)
{
	...
	msg_wrd = VXD_WR_REG_FIELD(msg_wrd, PVDEC_FW, DEVA_GENMSG, MSG_SIZE,
				   msg_size);
	msg_wrd = VXD_WR_REG_FIELD(msg_wrd, PVDEC_FW, DEVA_GENMSG, MSG_ID,
				   msg_id);
			      ...
			      void __iomem *reg_base)
{
	...
	if (val == PVDEC_FW_STATUS_PANIC  ||
	    val == PVDEC_FW_STATUS_ASSERT ||
			       ...
			       struct vxd_ena_params *ena_params)
{
	...
	msg_wrd = 0;
	msg_wrd = VXD_WR_REG_FIELD(msg_wrd, PVDEC_FW, DEVA_INIT, PFNT_DIV,
				   PVDEC_PFNT_DIV);
	VXD_WR_MSG_WRD(msg, PVDEC_FW_DEVA_INIT, PFNT_DIV, msg_wrd);

	/* firmware watchdog timeout value */
	msg_wrd = VXD_WR_REG_FIELD(msg_wrd, PVDEC_FW, DEVA_INIT, FWWDT_MS,
				   ena_params->fwwdt_ms);
		      ...
		      size_t msg_size)
{
	 ...
	 */
	if (ret == -EBUSY) {
		u32 flags = VXD_RD_REG_ABS(reg_base,
					   VLR_OFFSET + PVDEC_FW_FLAGS_OFFSET) |
					   PVDEC_FWFLAG_FAKE_COMPLETION;

		dev_dbg(dev,
			"comms space full, asking fw to send empty msg when space is available");

		VXD_WR_REG_ABS(reg_base, VLR_OFFSET + PVDEC_FW_FLAGS_OFFSET,
			       flags);
		ret = pvdec_check_comms_space(reg_base, msg_size, false);
	}
