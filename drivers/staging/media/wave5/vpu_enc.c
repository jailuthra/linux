			       ...
			       struct device *alloc_devs[])
{
	...
	if (inst->state == VPU_INST_STATE_NONE && q->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		ret = wave5_vpu_enc_open_api(inst, &open_param);
		if (ret) {
			dev_dbg(inst->dev->dev, "failed wave5_vpu_enc_open_api() : %d\n", ret);
			return -EINVAL;
		}
...
static int wave5_vpu_enc_open(struct file *filp)
{
