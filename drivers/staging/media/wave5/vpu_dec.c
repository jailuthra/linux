			 ...
			 struct device *alloc_devs[])
{
	...
	if (inst->state == VPU_INST_STATE_NONE && q->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		ret_code = wave5_vpu_dec_open_api(inst, &open_param);
		if (ret_code) {
			dev_dbg(inst->dev->dev, "wave5_vpu_dec_open_api failed: %d\n", ret_code);
			return -EINVAL;
		}
...
static int wave5_vpu_dec_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	if (inst->state == VPU_INST_STATE_OPEN) {
		...
		if (ret) {
			...
			dev_dbg(inst->dev->dev, "width %d height %d profile %d | minbuffer : %d\n",
...
static int wave5_vpu_dec_open(struct file *filp)
{
