...
int wave5_vpu_re_init(struct device *dev, void *firmware, uint32_t size)
{
	struct vpu_buf *vb;
	dma_addr_t code_base, temp_base;
...
static int wave5_set_enc_crop_info(s32 codec, struct enc_wave_param *param, int rot_mode,
						  ...
						  struct enc_open_param *pop)
{
	...
	if (vpu_inst->std == W_HEVC_ENC && low_delay && param->decoding_refresh_type == 1) {
		dev_info(dev, "WARN : dec_refresh_type (CRA) is supported if low delay GOP.\n");
