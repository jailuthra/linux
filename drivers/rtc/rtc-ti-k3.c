...
/*
 * This is the list of SoCs affected by the i2327 errata causing the RTC
 * state-machine to break if not unlocked fast enough during boot. These
 * SoCs must have the bootloader unlock this RTC device very early in the
 * boot-flow before we (Linux) can use this device.
 */
static const struct soc_device_attribute has_errata_i2327[] = {
...
static int k3rtc_configure(struct device *dev)
{
	 ...
	 */
	if (soc_device_match(has_errata_i2327)) {
		ret = k3rtc_check_unlocked(priv);
