				     ...
				     u32 flow_idx)
{
	...
	if (new_skb) {
		...
		skb->offload_fwd_mark = ndev_priv->offload_fwd_mark;
		skb_put(skb, pkt_len);
				     ...
				     struct cppi5_host_desc_t *desc)
{
	...
	dma_unmap_single(dev, buf_dma, buf_dma_len, DMA_TO_DEVICE);
...
static void am65_cpsw_nuss_tx_cleanup(void *data, dma_addr_t desc_dma)
{
	...
	skb = *(swdata);
	am65_cpsw_nuss_xmit_free(tx_chn, tx_chn->dma_dev, desc_tx);
...
err_free_descs:
	am65_cpsw_nuss_xmit_free(tx_chn, tx_chn->dma_dev, first_desc);
err_free_skb:
...
static int am65_cpsw_nuss_init_tx_chns(struct am65_cpsw_common *common)
{
	...
	for (i = 0; i < common->tx_ch_num; i++) {
		...
		tx_chn->dma_dev = k3_udma_glue_tx_dev_for_dma_api(tx_chn->tx_chn);
...
static int am65_cpsw_nuss_init_rx_chns(struct am65_cpsw_common *common)
{
	...
	rx_chn->dma_dev = k3_udma_glue_rx_dev_for_dma_api(rx_chn->rx_chn);
...
{
	if (ndev->netdev_ops == &am65_cpsw_nuss_netdev_ops_2g) {
		struct am65_cpsw_common *common = am65_ndev_to_common(ndev);
...
static void am65_cpsw_init_port_emac_ale(struct  am65_cpsw_port *port)
{
	...
	writel(0, port->port_base + AM65_CPSW_PORT_VLAN_REG_OFFSET);
...
static void am65_cpsw_init_port_switch_ale(struct am65_cpsw_port *port)
{
	...
	cpsw_ale_add_ucast(cpsw->ale, port->ndev->dev_addr,
			   HOST_PORT_NUM, ALE_SECURE | ALE_BLOCKED | ALE_VLAN, 0);
					...
					struct devlink_param_gset_ctx *ctx)
{
	...
	if (switch_en) {
		...
		cpsw_ale_control_set(cpsw->ale, HOST_PORT_NUM, ALE_BYPASS, 1);
...
static int am65_cpsw_nuss_register_devlink(struct am65_cpsw_common *common)
{
	...
	ret = devlink_params_register(common->devlink, am65_cpsw_devlink_params,
...
static int am65_cpsw_nuss_probe(struct platform_device *pdev)
{
	...
	eth_random_addr(common->switch_id);
