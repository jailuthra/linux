...
struct rpmsg_remotedev *rpmsg_remotedev_get_named_device(const char *device_name)
{
	...
	dev_dbg(dev, "%s: sending connect message\n", __func__);
...
static int rpmsg_kdrv_get_devices_cb(void *cb_data, void *req, int req_sz, void *resp, int resp_sz)
{
	...
	for (i = 0; i < info_resp->num_devices; i++) {
		...
		case RPMSG_KDRV_TP_DEVICE_TYPE_DISPLAY:
			priv->raw_device_data[cnt] = rpmsg_kdrv_display_device_init(rpdev, dev);
			break;
		default:
			dev_info(&rpdev->dev, "%s: new unsupported device: %s\n", __func__, dev->device_name);
			continue;
		}
...
static int __init rpmsg_kdrv_init(void)
{
	...
	ret = rpmsg_kdrv_display_driver_init();
	if (ret) {
		pr_err("failed display driver init: %d\n", ret);
		goto display_driver_fail;
	}
...
rpdrv_fail:
	rpmsg_kdrv_display_driver_fini();
display_driver_fail:
